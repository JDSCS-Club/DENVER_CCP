...
#define Tacho_MSB		0x0D
#define Tacho_LSB		0x0E

//static u16	gByTachoReadTime;

static unsigned int	gByTachoCount;
#endif 
...
#define CARID_CRA		0x01
#define CARID_CRB		0x02

#if PAC_OCC_PTT_CTRL
#define CNCS_CARNUM_MSB_INDEX	45//0x03
#define CNCS_CARNUM_LSB_INDEX	46//0x02
#define CNCS_OCC_STATE_INDEX	47//0x01
#else
#define CNCS_CARNUM_MSB_INDEX	0x02
#define CNCS_CARNUM_LSB_INDEX	0x01
#endif
...
#define OCC_PTTCTRL_SPKMODE			0x80
#if PAC_OCC_PTT_CTRL
static u8 VoIP_Call_ID[2];
static u8 VoIP_Call_State;
static u8 PTT_Ctrl_Flag;
#endif
u8 pre_send_data;
u8 gByMP3RxState;
u8 gByRxCount;
...
#if _COUPLE_MODIFY_
#define CAB_DEALY_TIME  		72*5
#define TL_RESP_CHECK_COUNT 	10
static u8 Reconfig_PortA_St;
static u8 Reconfig_PortB_St;
static u16 PortA_Check_Times;
static u16 PortB_Check_Times;
static u16 PortA_DecCheck_Times;
static u16 PortB_DecCheck_Times;
static u8 TL_Comm_Verify;
static u8 TL_CarNum_MSB,TL_CarNum_LSB;
#endif
...
void OCC_Ptt_Ctrl_SpeakerMode(void)
{
	...
	A_Port_Out[2] |= OCC_PTTCTRL_SPKMODE;	// close
	temp8 = Port_RW_Data(CPLD_WR, 2);
//#if PAC_OCC_PTT_CTRL//Test code CNCS 확인후 삭제.
//	VoIP_Call_State |= 0x08;
//#endif
}
...
int main(void)
{
	...
	//firmware version 1.04 & build date 16.06.23
	//Cab latch 적용 최종 version & build date
	//firmware version 1.05 & build date 17.03.06
	//Cab 전환시 CAB_ON bit가 변화되는 증상 보완.make_data cab_latch_cabon
	A_FirmVer[0] = 0x01;
	A_FirmVer[1] = 0x00;
	A_FirmVer[2] = 0x05;
	...
	*/
	...
	GPIO_Config(GPIO1, PI_TEST, GPIO_OUT_PP);
	...
	A_Port_In[0] = Port_RW_Data(CPLD_RD, 0);	// 0 0001
	A_Port_In[1] = Port_RW_Data(CPLD_RD, 1);	// 1 0010
	A_Port_In[2] = Port_RW_Data(CPLD_RD, 2);	// 2 0011
	A_Port_In[3] = Port_RW_Data(CPLD_RD, 3);	// 3 0100
	A_Port_In[4] = Port_RW_Data(CPLD_RD, 4);	// 4 0101
	...
	temp8 = Port_RW_Data(CPLD_WR, 0);
	temp8 = Port_RW_Data(CPLD_WR, 1);
	temp8 = Port_RW_Data(CPLD_WR, 2);
	...
	//Debug_msg(0xBC);
	//Debug_msg(A_FVol[I_V_NORISP]);
	//Debug_msg(A_FVol[I_V_SOFTISP]);
...
/* Car number 임의로 추가.
	if(T_Me_AB == 1){//CRA
		A_CarNum[0][0] = 0x40;//A_FSys[I_CARNUM1];
		A_CarNum[0][1] = 0x01;//A_FSys[I_CARNUM2];
	}
	else{//CRB
		A_CarNum[0][0] = 0x40;//A_FSys[I_CARNUM1];
		A_CarNum[0][1] = 0x02;//A_FSys[I_CARNUM2];
	}
*/	
	////////////////////// LOOP ////////////////////////////////////////////////////
	...
	while(1)
	{
		...
		if ((T_State & F_COUPLING_MODE) != F_COUPLING_MODE)
		{
			...
			if (TL_OK_Packet)
			{
				...
				if ((TL_Rx_Cmd & 0xC0) == 0xC0)		// coupling req(다른 차량의 couple 신호가 변하면 TX_Rx_CMD 0xC0를 or해서 넘어온다.)
				{
			...
			if (TL_Tx_Turn && (T_State & F_PAC_MASTER) == F_PAC_MASTER)	// && P_Coupling_ID > 1)
			{
				...
				#if _COUPLE_MODIFY_
					TL_CarNum_MSB = A_TL_CarNum[TL_Poll_ID][0];
					TL_CarNum_LSB = A_TL_CarNum[TL_Poll_ID][1]; 
					TL_Comm_Verify++;
					//Debug_msg(TL_CarNum_MSB+1);
					//Debug_msg(TL_CarNum_LSB);
				#endif
			...
			if (TL_Rx_Turn)
			{
				...
				#if _COUPLE_MODIFY_
					if((TL_CarNum_MSB == A_TL_CarNum[TL_Rx_SrcID][0])&&(TL_CarNum_LSB == A_TL_CarNum[TL_Rx_SrcID][1])){
						//Debug_msg(TL_CarNum_MSB);
						//Debug_msg(TL_CarNum_LSB);
						TL_Comm_Verify = RESET;
					}
				#endif
				}
				...
				if ((T_State & F_PAC_MASTER) != F_PAC_MASTER)
				{
					...
					if (TL_Rx_SrcID == TL_CAB_ID)//CAB key insert train 이부분은 실행하지 않음.CAB key가 없는 경우에는 실행됨.
					{
						...
						T_CI_Fault = T_CI_Fault & temploc;//이전 데이터가 존재하므로 이전 데이터 삭제후 아래 데이터로 업데이트.
...
*/
				...
				//Debug_msg(A_Port_Out[2]);
			}
		}
		MP3_SerialDataParsing();

	#if _COUPLE_MODIFY_
		if(TL_Comm_Verify>TL_RESP_CHECK_COUNT){
			T_State |= F_COUPLING_MODE;
			P_Coupling_Old = A_Port_ST[0] & 0x0F;
	...
	if ((T_State & F_CNCS_MODE) != F_CNCS_MODE)
	{
		...
		if (P_Coupling_ID > 1)								// 중련 o
		{
			if ((T_State & F_PAC_MASTER) == F_PAC_MASTER)	// 마스터 o	// Main
			{
				...
				{
				#if PAC_PROTOCOL_R10//20160310 VOIP 중련시 CRA에서 cancel dtmf tone 출력 하지않도록 추가.
					if (((A_Port_ST[0] & PI_ACAB) != PI_ACAB) && ((A_Port_ST[0] & PI_BCAB) != PI_BCAB)){
						A_Port_Out[1] |= PO_VOIPC_MAIN;//CRA, CRB에 CAB key가 없는 경우.
						temp8 = Port_RW_Data(CPLD_WR, 1);
					}
					else{
...
static	void	Port_Check(void)
{
...
#if _COUPLE_MODIFY_
	//GPIO1->PD ^= 0x8000;
	if((A_Port_In[0]&PI_ACAB) == PI_ACAB){
		if(Reconfig_PortA_St == 0){
			Reconfig_PortA_St = 1;
			if(T_Me_AB == CARID_CRA){//CRA
				PortA_Check_Times = CAB_DEALY_TIME;
			}
			else{//CRB
				PortA_Check_Times = 10;
			}
			A_Port_ST[0] &= ~PI_ACAB;
			//Debug_msg(1);
		}
		else if((PortA_Check_Times == 0)&&(Reconfig_PortA_St == 1)){
			A_Port_ST[0] |= PI_ACAB;
			Reconfig_PortA_St = 2;
			//Debug_msg(2);
		}
		else if(Reconfig_PortA_St == 1){
			A_Port_ST[0] &= ~PI_ACAB;
			PortA_DecCheck_Times = 0x80;		
			//Debug_msg(3);
		}
		else if(Reconfig_PortA_St == 2){
			A_Port_ST[0] |= PI_ACAB;
			PortA_Check_Times = 0;
			//Debug_msg(4);
		}
	}
	else if((A_Port_In[0]&PI_BCAB) == PI_BCAB){
		if(Reconfig_PortB_St == 0){
			Reconfig_PortB_St = 1;
			if(T_Me_AB == CARID_CRA){//CRA
				PortB_Check_Times = 10;
			}
			else{//CRB
				PortB_Check_Times = CAB_DEALY_TIME;
			}
			A_Port_ST[0] &= ~PI_BCAB;
			//Debug_msg(5);
		}
		else if((PortB_Check_Times == 0)&&(Reconfig_PortB_St == 1)){
			A_Port_ST[0] |= PI_BCAB;
			Reconfig_PortB_St = 2;
			//Debug_msg(6);
		}
		else if(Reconfig_PortB_St == 1){
			A_Port_ST[0] &= ~PI_BCAB;
			PortB_DecCheck_Times = 0x80;
			//Debug_msg(7);
		}
		else if(Reconfig_PortB_St == 2){
			A_Port_ST[0] |= PI_BCAB;
			PortB_Check_Times = 0;
			//Debug_msg(8);
		}
		...
		if(Reconfig_PortA_St != 0){
			PortA_DecCheck_Times--;
			if(PortA_DecCheck_Times == 0){
				A_Port_ST[0] &= ~PI_ACAB;
				Reconfig_PortA_St = 0;
				PortA_Check_Times = 0;
				//Debug_msg(9);
		...
		if(Reconfig_PortB_St != 0){
			PortB_DecCheck_Times--;
			if(PortB_DecCheck_Times == 0){
				A_Port_ST[0] &= ~PI_BCAB;
				Reconfig_PortB_St = 0;
				PortB_Check_Times = 0;
				//Debug_msg(11);
	...
	if (CL_State == ST_CL_POllING)
	{
		...
		*/
				...
				*/
				...
				if ((TP_Broad_Device & F_PEI_AB) != F_PEI_AB)		// 홀수 탐색
				{
					...
					if ((AP_Train_ST[T_PEI_Num][7] & 0x0F) > 0 && (T_PEI_Play[T_PEI_Num] & 0x0F) == 0)
					{
						...
						temp8 = Port_RW_Data(CPLD_WR, 0);
						//20160106 PEI recode 기능 포트 제어.
						//A_Port_Out[2] |= 0x10;//CSP audio path off
						//temp8 = Port_RW_Data(CPLD_WR, 2);
		...
		if ((TP_Broad_Device & F_PEI_ING) == F_PEI_ING)
		{
			...
			if (T_PEI_Cnt == 1)
			{
				...
				#if PAC_PROTOCOL_R10
					//20160112 PEI anouncement가 end되면 voice recording이 되지 않으므로 
					//DPO cancel이 들어 인식되면 clear하도록 처리함.
					//P_Broad_Device &= ~P_PAC_PEI_B;
					//A_Port_Out[2] &= 0xEF;//CSP audio path on
					//temp8 = Port_RW_Data(CPLD_WR, 2);
				#endif 	
		...
		else
		{
			...
			if ((A_Port_ST[1] & PI_IC_VOICE) != PI_IC_VOICE)
			{
				if (T_IC_Cnt == 0 && (TP_Broad_Device & F_IC_ING) != F_IC_ING)
				{
					...
					//20160324 IC 통화 녹음을 위해 PEI flag를 이용함.
					P_Broad_Device |= P_PAC_PEI_B;
		...
		else if ((T_Broad & F_AUTO_ING) == F_AUTO_ING)
		{
			...
			if (TC_Broad[2] == 20)
			{
				...
				Set_Vol_I2C(1);//CNCS UI에서 volume제어함.
		...
		else if ((A_Port_ST[1] & PI_INT_PA_IN) != PI_INT_PA_IN || (A_Port_ST[1] & PI_EXT_PA_IN) != PI_EXT_PA_IN)
		{
			...
			if((P_CNCS_PA & P_CNCS_OCC_PA) == P_CNCS_OCC_PA){
				...
				Set_Vol_I2C(1);//170405 DI 입력시 동작하게 수정.
			}
			else if((P_CNCS_PA & P_CNCS_READY) == P_CNCS_READY){
				P_Broad_Device |= P_PAC_AUTO_B;
				//T_Broad |= F_AUTO_START;//170405 DI 입력시 동작하게 수정. 
				//Debug_msg('o');
		...
		*/
		...
		#if PAC_PROTOCOL_R10
		//20160112 PEI voice recording end 시점을 이동함.
			if((P_Broad_Device&P_PAC_PEI_B)==P_PAC_PEI_B){
				P_Broad_Device &= ~P_PAC_PEI_B;
			}
		#endif
		}
		/*
		if(((P_CNCS_PA & P_CNCS_OCC_PA)==0)&&(P_Broad_Device&P_PAC_OCC_B)){
			A_Port_Out[1] |= (PO_INT_PA_OUT + PO_EXT_PA_OUT);
			//20160106실외 방송은 제외함.
			//A_Port_Out[1] |= (PO_INT_PA_OUT);
...
static	u8		Port_RW_Data(u8 gbn, u8 sel)
{
	...
	if (gbn == CPLD_RD)	// read
	{
		switch (sel)
		{
		case 0x00://CPLD input data = 0x01
			GPIO2->PD |= PO_IO_SEL_0;
			break;

		case 0x01://CPLD input data = 0x02
			GPIO2->PD |= PO_IO_SEL_1;
			break;

		case 0x02://CPLD input data = 0x03
			GPIO2->PD |= PO_IO_SEL_1 + PO_IO_SEL_0;
			break;

		case 0x03://CPLD input data = 0x04
			GPIO2->PD |= PO_IO_SEL_2;
			break;

		case 0x04://CPLD input data = 0x05
			GPIO2->PD |= PO_IO_SEL_2 + PO_IO_SEL_0;
			break;
		//201512011 : tachometer MSB register read
		case 0x0D://CPLD input data = 0x0D
			GPIO2->PD |= PO_IO_SEL_3 + PO_IO_SEL_2 + PO_IO_SEL_0;
			break;
		//201512011 : tachometer LSB register read
		case 0x0E://CPLD input data = 0x0E
			GPIO2->PD |= PO_IO_SEL_3 + PO_IO_SEL_2 + PO_IO_SEL_1;
			break;
		case 0x0F://CPLD input data = 0x0E
			GPIO2->PD |= PO_IO_SEL_3 + PO_IO_SEL_2 + PO_IO_SEL_1 + PO_IO_SEL_0;
			break;
		case 0x0C://CPLD input data = 0x0E
			GPIO2->PD |= PO_IO_SEL_3 + PO_IO_SEL_2;// + PO_IO_SEL_1 + PO_IO_SEL_0;
			break;
		}
...
void	CL_Timer(void)
{
	...
	//20151211 : Tachometer data read test 
	//temp16 =(u16)Port_RW_Data(CPLD_RD,Tacho_MSB);
	//temp16 = (temp16 & 0x00FF)<<8;
	//temp16 |= ((u16)(Port_RW_Data(CPLD_RD,Tacho_LSB))&0x00FF);
#if 0//20160309 Tacho data read delete	
	u16		TempTacho;
	if(gByTachoReadTime>0) {
		gByTachoReadTime--;
	}
	else{
		TempTacho = Port_RW_Data(CPLD_RD,Tacho_MSB);
		TempTacho = (TempTacho & 0x00FF)<<8;
		//Debug_msg(i);
		i = Port_RW_Data(CPLD_RD,Tacho_LSB);
		TempTacho |= ((u16)i&0x00FF);
		//Debug_msg(i);
		i = Port_RW_Data(CPLD_RD,0x0C);
		//Debug_msg(i);

		gByTachoCount = gByTachoCount + (unsigned int)TempTacho;
		gByTachoReadTime = 100;
	}
#endif	
...
#if 0
			Debug_msg(0xff);
			Debug_msg((tempans&0xff00)>>8);
			if((tempans&0x00ff) == 0){
				Debug_msg(0x10);
			}
			else{
				Debug_msg(tempans&0x00ff);
			}
#endif
...
#if _COUPLE_MODIFY_
	//TL 응답에 문제가 발생함.
	if(PortA_Check_Times>0){
		PortA_Check_Times--;
	}
	if(PortB_Check_Times>0){
		PortB_Check_Times--;
	}
#endif
}
...
static	void	Make_Data(u8 vid)
{
	...
	else
	{
		...
		case VBPAC:
			if ((T_State & F_PAC_MAIN) == F_PAC_MAIN && A_VDev_MS[AI_PACM] == vid)	// 내가 메인 메인에게 전체패킷
			{
			...
			#if CAB_LATCH_CABON
				if(vid == VAPAC){
					if((P_IOport & P_PAC_BCAB) == P_PAC_BCAB){
						p_ioport_temp = P_IOport & ~P_PAC_BCAB;
					}
					else{
						p_ioport_temp = P_IOport;
					}
					//Debug_msg(0x31);
				}
				else if(vid == VBPAC){
					if((P_IOport & P_PAC_ACAB) == P_PAC_ACAB){
						p_ioport_temp = P_IOport & ~P_PAC_ACAB;
					}
					else{
						p_ioport_temp = P_IOport;
					}
			...
			#if PAC_PROTOCOL_R10
				#if PAC_OCC_PTT_CTRL
				CL_Tx_Data[cnt++] = P_Broad_Occ | VoIP_Call_State;
				#else
				...
				//gByTachoCount = T_Ans_Avr*100/126;//자동 방송시에만 변함.
				//gByTachoCount = ((u32)A_Ans_Val[0]*100)/126;//실시간 변화값.
				gByTachoCount = (u32)(A_Ans_Val[AI_Ans-1]);
				AP_POdo[0] = (gByTachoCount & 0xFF000000)>>24;
				AP_POdo[1] = (gByTachoCount & 0x00FF0000)>>16;
				AP_POdo[2] = (gByTachoCount & 0x0000FF00)>>8;
				AP_POdo[3] = (gByTachoCount & 0x000000FF);
			#endif
			...
			#if CAB_LATCH_CABON
				ap_trainnum_sum = AP_TrainNum_Backup[0]+AP_TrainNum_Backup[1]+AP_TrainNum_Backup[2];//booting후 초기화 데이터 연산.
				if(ap_trainnum_sum == 0){//초기 데이터가 업데이트가 되지 않으면 초기 backup data를 설정한다.
					CL_Tx_Data[cnt++] = AP_TrainNum[0]; 	//10
					CL_Tx_Data[cnt++] = AP_TrainNum[1];
					CL_Tx_Data[cnt++] = AP_TrainNum[2];
					AP_TrainNum_Backup[0] = AP_TrainNum[0]; 	//10
					AP_TrainNum_Backup[1] = AP_TrainNum[1];
					AP_TrainNum_Backup[2] = AP_TrainNum[2];
				}
				else{//backup data가 업데이트 된 이후 ccp train number가 '0'이면 이전 backup data를 전송 한다.
					ap_trainnum_sum = AP_TrainNum[0]+AP_TrainNum[1]+AP_TrainNum[2];
					if(ap_trainnum_sum == 0){
						CL_Tx_Data[cnt++] = AP_TrainNum_Backup[0]; 	//10
						CL_Tx_Data[cnt++] = AP_TrainNum_Backup[1];
						CL_Tx_Data[cnt++] = AP_TrainNum_Backup[2];
					}
					else{
						CL_Tx_Data[cnt++] = AP_TrainNum[0]; 	//10
						CL_Tx_Data[cnt++] = AP_TrainNum[1];
			...
			#endif
			#if PAC_OCC_PTT_CTRL
				CL_Tx_Data[cnt++] = VoIP_Call_ID[0];
				CL_Tx_Data[cnt++] = VoIP_Call_ID[1];
			#endif
			#endif
			}
			CL_Tx_Len = cnt;
		...
		case VBPID21:
			...
			CL_Tx_Len = cnt;
		//====================================================
		//VoIP protocol 확인후 삭제.160122 voip protocol 학인됨.
		#if 0
			if(vid == VAVOIPC){
				if(gByAVoIPComm == 0){
					gByAVoIPComm = 1;
...
static 	void Car_Number_Process(u8 vvsid)
{
	u32		i;
	u8		carh, carl;
	u16 	cncs_car_msb,cncs_car_lsb;

#if PAC_PROTOCOL_R10
	//if (CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_MSB_INDEX] == 0 && CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_LSB_INDEX] == 0)
   	cncs_car_msb = CNCS_CARNUM_MSB_INDEX + CL_Rx_Data[43];
   	cncs_car_lsb = CNCS_CARNUM_LSB_INDEX + CL_Rx_Data[43];
	if (CL_Rx_Data[cncs_car_msb] == 0 && CL_Rx_Data[cncs_car_lsb] == 0)
	{
	}
#else
	...
	else
	{
	...
	#endif
		{
			...
			{
			#if PAC_PROTOCOL_R10
				//A_CarNum[AI_CarNum][0] = CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_MSB_INDEX];
				//A_CarNum[AI_CarNum][1] = CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_LSB_INDEX];
				cncs_car_msb = CNCS_CARNUM_MSB_INDEX + CL_Rx_Data[43];
				cncs_car_lsb = CNCS_CARNUM_LSB_INDEX + CL_Rx_Data[43];
				A_CarNum[AI_CarNum][0] = CL_Rx_Data[cncs_car_msb];
				A_CarNum[AI_CarNum][1] = CL_Rx_Data[cncs_car_lsb];
			#else
			...
			{
			#if PAC_PROTOCOL_R10
				//carh = CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_MSB_INDEX];
				//carl = CL_Rx_Data[CL_Rx_Index-CNCS_CARNUM_LSB_INDEX];
				cncs_car_msb = CNCS_CARNUM_MSB_INDEX + CL_Rx_Data[43];
				cncs_car_lsb = CNCS_CARNUM_LSB_INDEX + CL_Rx_Data[43];
				carh = CL_Rx_Data[cncs_car_msb];
				carl = CL_Rx_Data[cncs_car_lsb];
			#else
				carh = CL_Rx_Data[2];
				carl = CL_Rx_Data[3];
			#endif
...
//	#endif
	...
	case VBPAC:
		...
		else
		{
			if ((vsid == VAPAC && vdid == VAPAC) || (vsid == VBPAC && vdid == VBPAC))
			{
				...
				AP_TrainNum[0] = CL_Rx_Data[10];		//10
				AP_TrainNum[1] = CL_Rx_Data[11];
				AP_TrainNum[2] = CL_Rx_Data[12];
				
				ESpk_State = CL_Rx_Data[28];
				CISpk_State = CL_Rx_Data[29];
			#if PAC_PROTOCOL_R10
	...
	case VBCCP:
		...
		if (P_Coupling_ID == 0 || (T_State & F_PAC_MASTER) == F_PAC_MASTER)
		{
			...
			if(T_Train_Len>1){//중련시에 적용됨.		
				if(((P_IOport&P_PAC_ACAB)==P_PAC_ACAB)||((P_IOport&P_PAC_BCAB)==P_PAC_BCAB)){
					ESpk_State = CL_Rx_Data[19];
					CISpk_State = CL_Rx_Data[20];
				}
				else{
					ESpk_State = ESpk_State;
					CISpk_State = CISpk_State;
				}
			}
			else{//단차 상태에서 적용됨.
				ESpk_State = CL_Rx_Data[19];
			...
			if ((ESpk_State & ESP_NORMAL) == ESP_NORMAL && (ESpk_ST_Me & ESP_NORMAL) != ESP_NORMAL)
			{
				//Set_Vol_I2C(2);
				//Set_Vol_I2C(3);
				Set_Vol_I2C(ESPR_SCL_VOL2);
				Set_Vol_I2C(ESPL_SCL_VOL3);
	...
	case VBCNCS:
		...
		if(((P_IOport&P_PAC_ACAB)==P_PAC_ACAB)||((P_IOport&P_PAC_BCAB)==P_PAC_BCAB)){//20160317_CAB key가 존재 할경우.
	...
	#if PAC_OCC_PTT_CTRL
		//if((CL_Rx_Data[CL_Rx_Index-CNCS_OCC_STATE_INDEX] & 0x02) == 0x02){
		//	if((CL_Rx_Data[CL_Rx_Index-CNCS_OCC_STATE_INDEX] & 0x04) == 0x04){
		if((CL_Rx_Data[CNCS_OCC_STATE_INDEX] & 0x02) == 0x02){
			if((CL_Rx_Data[CNCS_OCC_STATE_INDEX] & 0x04) == 0x04){
				VoIP_Call_State |= 0x08;
				if(PTT_Ctrl_Flag == 1){
					OCC_Ptt_Ctrl_SpeakerMode();
					PTT_Ctrl_Flag = 0;
				}
			}
			else{
	...
	#if PAC_OCC_PTT_CTRL
		if((CL_Rx_Data[0] & 0x01) == 0x00){
			if((CL_Rx_Data[0] & 0x06) == 0x06){
				if((VoIP_Call_State & 0x08) == 0x08){
					VoIP_Call_State = 0x06 | 0x08;
				}
				else{
					VoIP_Call_State = 0x06;
				}
			}
			else if((CL_Rx_Data[0] & 0x04) == 0x04){
				VoIP_Call_State = 0x02;
			}
			else if((CL_Rx_Data[0] & 0x02) == 0x02){
				VoIP_Call_State = 0x04;
			}
		}
		else{
			VoIP_Call_State = 0x00;
			PTT_Ctrl_Flag = 0;
		}
		
		if(T_Me_AB == CARID_CRA){
			VoIP_Call_ID[0] = CL_Rx_Data[1]; 
			VoIP_Call_ID[1] = CL_Rx_Data[2]; 
		}
		else{
			VoIP_Call_ID[0] = CL_Rx_Data[1]; 
			VoIP_Call_ID[1] = CL_Rx_Data[2]; 
...
static	void	Set_Vol_I2C(u8 sel)
{
	...
	//case 5:		// TL OUT	0101
	case TLOUT_SCL_VOL4: 	// TL OUT	0100
		I2C_Val = A_FVol[I_V_TLOUT];
		//GPIO1->PD |= PO_SCL_SEL_2 + PO_SCL_SEL_0;//0101
		GPIO1->PD |= PO_SCL_SEL_2;
		break;

	//case 6:		// TL IN	0110	// 11k
	case OCC_SCL_VOL5: 	// TL IN	0101	// 11k
		I2C_Val = 31;
		//GPIO2->PD |= PO_SCL_SEL_3;
	...
	//case 7:		// OCC IN	0111
	case DPO_SCL_VOL6: 	// OCC IN	0110
		//I2C_Val = A_FVol[I_V_OCC];
		//GPIO1->PD |= PO_SCL_SEL_2 + PO_SCL_SEL_1 + PO_SCL_SEL_0;
	#if DPO_BASS_ENABLE	
		I2C_Val = A_FVol[I_V_DPO]+0x40;
	#else
		I2C_Val = A_FVol[I_V_DPO];//+0x40;
	#endif
		GPIO1->PD |= PO_SCL_SEL_2 + PO_SCL_SEL_1;
	...
	//case 8:		// DPO IN	1000
	case MP3_SCL_VOL7: 	// DPO IN	0111
		//I2C_Val = A_FVol[I_V_DPO];
		//GPIO2->PD |= PO_SCL_SEL_3;
		I2C_Val = A_FVol[I_V_AUTO];
		GPIO1->PD |= PO_SCL_SEL_2 + PO_SCL_SEL_1 + PO_SCL_SEL_0;
		break;

	//case 9:		// PEI IN	1001
	case TLIN_SCL_VOL8: 	// PEI IN	1000
		//I2C_Val = A_FVol[I_V_PEI];
		//GPIO2->PD |= PO_SCL_SEL_3;
		//GPIO1->PD |= PO_SCL_SEL_0;
		I2C_Val = A_FVol[I_V_TLIN]+25;
		GPIO2->PD |= PO_SCL_SEL_3;
		break;
	//case 4:		// CSP	0100
	case CSP_SCL_VOL9:	// CSP	1001
...
void	TL_TxHandler(void)
{
	...
	case TTX_ST_DUMMY2:
		...
		if(TL_Tx_Rep_Dummy == RESET){
			TL_Tx_Rep_Dummy = SET;
		}
		else{
			TL_Tx_Rep_Dummy = RESET;
		}
		break;
...
#if _COUPLE_MODIFY_
	Reconfig_PortA_St = RESET;
	Reconfig_PortB_St = RESET;
	PortA_Check_Times = RESET;
	PortB_Check_Times = RESET;
	PortA_DecCheck_Times = RESET;
	PortB_DecCheck_Times = RESET;
	TL_Comm_Verify = RESET;
	TL_CarNum_MSB = RESET;
	TL_CarNum_LSB = RESET;
#endif
