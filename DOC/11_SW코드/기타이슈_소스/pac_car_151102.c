		...
		if ((T_State & F_COUPLING_MODE) != F_COUPLING_MODE)
		{
			...
			if (TL_Rx_Turn)
			{
				...
				if (((T_State & F_PAC_MASTER) == F_PAC_MASTER) && (P_Coupling_ID != TL_Rx_SrcID))	// 단지 업데이트
				{
					...
					temploc = (0x03 << (TL_Rx_SrcID - 1));
					T_CI_Fault = T_CI_Fault & ~(temploc);
					tempval = TL_Rx_Data[72] & temploc;
					T_CI_Fault |= tempval;
				}
			}	// rxturn
			...
			if ((T_State & F_PAC_MAIN) == F_PAC_MAIN)
			{
				...
				else
				{
					if (A_ErrDevs[CL_Tx_ID]++ > 1)
					{
						...
						case VAVOIPC:	AP_Train_ST[0][1] |= LIVE_AVOIPC;	break;
						case VBVOIPC:	
							AP_Train_ST[0][1] |= LIVE_BVOIPC;	break;
						case VACNCS:
	...
	else
	{
		...
		case VBPAC:
			if ((T_State & F_PAC_MAIN) == F_PAC_MAIN && A_VDev_MS[AI_PACM] == vid)	// 내가 메인 메인에게 전체패킷
			{
				...
				CL_Tx_Data[cnt++] = AP_SP_Vec[1];
				for (i=0 ; i<10 ; i++)
				{
					CL_Tx_Data[cnt++] = AP_St_Skip[i];
				}
				//Debug_msg(0xAB);
				for (i=0 ; i<8 ; i++)
				{
				#if PAC_PROTOCOL_R10
					CL_Tx_Data[cnt++] = AP_CID_Tx[i];//send to CCP car line data
				#else
					CL_Tx_Data[cnt++] = AP_CID_Rx[i];
				#endif
					//Debug_msg(AP_CID_Tx[i]);
				}
...
static 	void Car_Number_Process(u8 vvsid)
{
	...
	else
	{
	...
	#endif
		{
			...
			{
			#if PAC_PROTOCOL_R10
				temp8 = (CL_Rx_Data[CL_Rx_Index-1] & 0x0F);
				if(temp8%2){//CRA alway odd number car line data적용.
					A_CarNum[AI_CarNum][0] = CL_Rx_Data[CL_Rx_Index-2];
					A_CarNum[AI_CarNum][1] = CL_Rx_Data[CL_Rx_Index-1];
				}
				else{//CRA even data 입력시 이전 데이터로 치환.
					if(AI_CarNum == 0){
						A_CarNum[AI_CarNum][0] = A_CarNum[AI_CarNum][0];
						A_CarNum[AI_CarNum][1] = A_CarNum[AI_CarNum][1];
					}
					else{
						A_CarNum[AI_CarNum][0] = A_CarNum[AI_CarNum-1][0];
						A_CarNum[AI_CarNum][1] = A_CarNum[AI_CarNum-1][1];
					}
				}
			#else
			...
			#if PAC_PROTOCOL_R10
				temp8 = (CL_Rx_Data[CL_Rx_Index-1] & 0x0F);
				if(temp8%2){//CRB odd data입력시.
					if(AI_CarNum == 0){
						carh = A_CarNum[AI_CarNum][0];
						carl = A_CarNum[AI_CarNum][1];
					}
					else{
						carh = A_CarNum[AI_CarNum-1][0];
						carl = A_CarNum[AI_CarNum-1][1];
					}
				}
				else{//CRB even data입력.
		...
		else
		{
			...
			#if PAC_PROTOCOL_R10
				temp8 = (CL_Rx_Data[CL_Rx_Index-1] & 0x0F);
				if(temp8%2){//CRA odd data입력시.
					carh = CL_Rx_Data[CL_Rx_Index-2];
					carl = CL_Rx_Data[CL_Rx_Index-1];
				}
				else{//CRA even data입력.
					if(AI_CarNum == 0){
						carh = A_CarNum[AI_CarNum][0];
						carl = A_CarNum[AI_CarNum][1];
					}
			...
			else
			{
			#if PAC_PROTOCOL_R10
				temp8 = (CL_Rx_Data[CL_Rx_Index-1] & 0x0F);
				if(temp8%2){//CRB odd data 입력시 이전 데이터로 치환.
					if(AI_CarNum == 0){
						A_CarNum[AI_CarNum][0] = A_CarNum[AI_CarNum][0];
						A_CarNum[AI_CarNum][1] = A_CarNum[AI_CarNum][1];
					}
				...
				else{//CRB alway even number car line data적용.
					A_CarNum[AI_CarNum][0] = CL_Rx_Data[CL_Rx_Index-2];
					A_CarNum[AI_CarNum][1] = CL_Rx_Data[CL_Rx_Index-1];
				}					
			#else
