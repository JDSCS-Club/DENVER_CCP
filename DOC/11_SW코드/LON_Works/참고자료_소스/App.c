...
/*******************************************************************************
 ...
 RefImpToAlt(), AltToRefImp(): Functions to transform data between a format
 often used by 709.1 applications (such as msg_in, msg_out, resp_in etc) and
 the corresponding structures for reference implementation (msgIn, msgOut, etc).
...
#else
#define REF_IMP_TO_ALT(a,b)
#define ALT_TO_REF_IMP(a,b)
#endif
                           ...
                           APDU            *apduPtr)
{
   ...
   if (gp->respIn.len <= gp->appInBufSize)
   {
      ...
      gp->respReceive = TRUE;
#if defined(ALTERNATE_STRUCTURES_NEEDED)
           REF_IMP_TO_ALT(RESP_IN, &resp_in);
#endif
   }
   else
   {
...
void GoUnconfigured(void)
{
   ...
   for (i = 0; i < numDomains; i++)
   {
      ...
      memcpy(eep->domainTable[i].key, "NO_KEY", AUTH_KEY_LEN);
   }
}
