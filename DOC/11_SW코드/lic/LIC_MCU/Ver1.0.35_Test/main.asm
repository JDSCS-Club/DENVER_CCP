...
;* Date/Time created: Sat Sep 19 09:27:39 2015                                *
;******************************************************************************
...
_ConvAsc2Hex:
	...
	.line	1
;----------------------------------------------------------------------
; 182 | UCHAR ConvAsc2Hex(char chDat)                                          
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	...
	.line	4
;----------------------------------------------------------------------
; 185 | if(chDat >= '0' && chDat <= '9') nBuf = chDat-'0';                     
; 186 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      48,r0
...
L13:        
	.line	6
;----------------------------------------------------------------------
; 187 | if(chDat >= 'a' && chDat <= 'f') nBuf = ((chDat-'a') + 10);
;     |                                                                        
; 188 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
...
L16:        
	.line	8
;----------------------------------------------------------------------
; 189 | if(chDat >= 'A' && chDat <= 'F') nBuf = ((chDat-'A') + 10);            
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      65,r0
        blt       L19
...
_FunConvAscHex:
	...
	.line	1
;----------------------------------------------------------------------
; 197 | int FunConvAscHex(char *InchData,UCHAR *OuthexData,int Len)            
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
L23:        
	.line	7
;----------------------------------------------------------------------
; 203 | OuthexData[sCnt] = (ConvAsc2Hex(InchData[i++])<<4)&0xF0;               
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *+fp(1),ar0
        ldiu      *-fp(2),ir0
	...
	.line	8
;----------------------------------------------------------------------
; 204 | OuthexData[sCnt] |= ConvAsc2Hex(InchData[i++])&0x0F;                   
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *+fp(1),ar0
        addi      ar0,r0
...
;----------------------------------------------------------------------
	...
	.line	3
;----------------------------------------------------------------------
; 216 | if(nCh >= '0' && nCh <= '9') return TRUE;                              
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      48,r0
        blt       L30
...
;----------------------------------------------------------------------
	...
	.line	4
;----------------------------------------------------------------------
; 226 | if(nCh >= '0' && nCh <= '9') nBuf = nCh-'0'; else return 0;            
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      48,r0
        blt       L36
...
_ConvHex2Asc:
	...
	.line	1
;----------------------------------------------------------------------
; 233 | char ConvHex2Asc(UCHAR btCh)                                           
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	...
	.line	4
;----------------------------------------------------------------------
; 236 | if(btCh >= 0 && btCh <= 9) chBuf = btCh+'0';                           
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      9,r0
        bhi       L42
...
L42:        
	.line	5
;----------------------------------------------------------------------
; 237 | else if(btCh >= 10 && btCh <= 15) chBuf = (btCh-10)+'A';               
;----------------------------------------------------------------------
        ldiu      *-fp(2),r0
        cmpi      10,r0
        blo       L45
...
;----------------------------------------------------------------------
	...
	.line	3
;----------------------------------------------------------------------
; 245 | return ((((nCh / 0x10) & 0x0F) << 4) | ((nCh % 0x10) & 0x0F));         
;----------------------------------------------------------------------
        ldiu      *-fp(2),r3
        ldiu      r3,r0
        ldiu      r0,r2
...
_FunConvHexAsc:
	...
	.line	1
;----------------------------------------------------------------------
; 248 | int FunConvHexAsc(UCHAR *InhexData,char *OUTAscData,int Len)           
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
L52:        
	.line	7
;----------------------------------------------------------------------
; 254 | OUTAscData[sCnt++] = ConvHex2Asc(BYTE_H(InhexData[i]));                
;----------------------------------------------------------------------
        ldiu      *-fp(2),ir0
        ldiu      *+fp(1),ar0
        lsh3      r5,*+ar0(ir0),r0
	...
	.line	8
;----------------------------------------------------------------------
; 255 | OUTAscData[sCnt++] = ConvHex2Asc(BYTE_L(InhexData[i]));                
;----------------------------------------------------------------------
        ldiu      *+fp(1),ar0
        ldiu      *-fp(2),ir0
        and3      r4,*+ar0(ir0),r0
...
_Dec2Str:
	...
	.line	1
;----------------------------------------------------------------------
; 262 | void Dec2Str(char *pBuf,int nDat)                                      
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      34,sp
...
;----------------------------------------------------------------------
	...
	.line	8
;----------------------------------------------------------------------
; 269 | szBuf[nPos++] = (nDat/1000000000%10)+'0';                              
;----------------------------------------------------------------------
        ldiu      @CL14,r1
        ldiu      *-fp(3),r0
        call      DIV_I30
	...
	.line	9
;----------------------------------------------------------------------
; 270 | szBuf[nPos++] = (nDat/100000000%10)+'0';                               
;----------------------------------------------------------------------
        ldiu      @CL15,r1
        ldiu      *-fp(3),r0
        call      DIV_I30
	...
	.line	10
;----------------------------------------------------------------------
; 271 | szBuf[nPos++] = (nDat/10000000%10)+'0';                                
;----------------------------------------------------------------------
        ldiu      @CL16,r1
        ldiu      *-fp(3),r0
        call      DIV_I30
	...
	.line	11
;----------------------------------------------------------------------
; 272 | szBuf[nPos++] = (nDat/1000000%10)+'0';                                 
;----------------------------------------------------------------------
        ldiu      @CL17,r1
        ldiu      *-fp(3),r0
        call      DIV_I30
	...
	.line	12
;----------------------------------------------------------------------
; 273 | szBuf[nPos++] = (nDat/100000%10)+'0';                                  
;----------------------------------------------------------------------
        ldiu      *-fp(3),r0
        ldiu      @CL18,r1
        call      DIV_I30
	...
	.line	13
;----------------------------------------------------------------------
; 274 | szBuf[nPos++] = (nDat/10000%10)+'0';                                   
;----------------------------------------------------------------------
        ldiu      *-fp(3),r0
        ldiu      10000,r1
        call      DIV_I30
...
;----------------------------------------------------------------------
	...
	.line	20
;----------------------------------------------------------------------
; 281 | for(i=0;i<nPos;i++) if(szBuf[i]!='0') break;                           
;----------------------------------------------------------------------
        sti       r0,*+fp(1)
        cmpi      *+fp(2),r0
        bge       L60
...
_StrCat:
	...
	.line	1
;----------------------------------------------------------------------
; 290 | void StrCat(char *dest, const char *src)                               
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
_StrLen:
	...
	.line	1
;----------------------------------------------------------------------
; 300 | int StrLen(const char *string)                                         
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
_StrCpy:
	...
	.line	1
;----------------------------------------------------------------------
; 310 | void StrCpy(char *dest, const char *src)                               
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
_StrCmp:
	...
	.line	1
;----------------------------------------------------------------------
; 319 | int StrCmp(const char *string1, const char *string2)                   
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      3,sp
...
;----------------------------------------------------------------------
	...
	.line	7
;----------------------------------------------------------------------
; 339 | vsprintf(szBuf, format, args);                                         
;----------------------------------------------------------------------
        ldiu      r0,r1
        ldiu      fp,r0
        push      r1
	...
	.line	9
;----------------------------------------------------------------------
; 341 | xr16l784_Send(XR16L784_1CHL,(UCHAR *)szBuf,strlen(szBuf));             
;----------------------------------------------------------------------
        ldiu      fp,r0
        addi      2,r0
        push      r0
...
_FirmwareVersionStrCmp:
	...
	.line	1
;----------------------------------------------------------------------
; 347 | BOOL FirmwareVersionStrCmp(char *pObj,char *pSrc,int nLen)             
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 351 | for(i=0;i<nLen;i++) if(pObj[i] != pSrc[i]) {bFlag = FALSE; break;}     
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        cmpi      *-fp(4),r0
...
_GetFirmwareVersion:
	...
	.line	1
;----------------------------------------------------------------------
; 360 | int GetFirmwareVersion(UCHAR nVerBuildData)                            
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      3,sp
...
L97:        
	.line	9
;----------------------------------------------------------------------
; 368 | pVersionString = (char *)m_pBoardIdInfo[i];                            
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0
        ldiu      @CL19,ar0
        ldi       *+ar0(ir0),r0
	...
	.line	10
;----------------------------------------------------------------------
; 369 | if(pVersionString == NULL) break;                                      
;----------------------------------------------------------------------
        beq       L106
;*      Branch Occurs to L106 
	.line	12
;----------------------------------------------------------------------
; 371 | if(WORD_L(nVerBuildData) == 1)                                         
;----------------------------------------------------------------------
        ldiu      255,r0
        and       *-fp(2),r0
        cmpi      1,r0
...
;*      Branch Occurs to L102 
	.line	14
;----------------------------------------------------------------------
; 373 | if(FirmwareVersionStrCmp("Version",pVersionString,7))                  
;----------------------------------------------------------------------
        ldiu      7,r0
        push      r0
        ldiu      *+fp(3),r1
...
;*      Branch Occurs to L105 
	.line	16
;----------------------------------------------------------------------
; 375 | nVer = ConvAsc2Dec(pVersionString[8])*1000+ConvAsc2Dec(pVersionString[1
...
L102:        
	.line	20
;----------------------------------------------------------------------
; 379 | else if(WORD_L(nVerBuildData) == 2)                                    
;----------------------------------------------------------------------
        ldiu      255,r0
        and       *-fp(2),r0
        cmpi      2,r0
...
;*      Branch Occurs to L105 
	.line	22
;----------------------------------------------------------------------
; 381 | if(FirmwareVersionStrCmp("Date",pVersionString,4))                     
;----------------------------------------------------------------------
        ldiu      4,r0
        ldiu      @CL21,r1
        push      r0
...
;*      Branch Occurs to L105 
	.line	24
;----------------------------------------------------------------------
; 383 | nVer = ConvAsc2Dec(pVersionString[7])*100000+ConvAsc2Dec(pVersionString
;     | [8])*10000+ConvAsc2Dec(pVersionString[10])*1000+                       
; 384 |        ConvAsc2Dec(pVersionString[11])*100+ConvAsc2Dec(pVersionString[1
...
_Make1ByteBcc:
	...
	.line	1
;----------------------------------------------------------------------
; 396 | UCHAR Make1ByteBcc(UCHAR *pDat,int nLen)                               
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	.line	4
;----------------------------------------------------------------------
; 399 | UCHAR btBcc = 0x00;                                                    
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(2)
	.line	5
;----------------------------------------------------------------------
; 400 | for(i=0;i<nLen;i++)     btBcc ^= pDat[i];                              
;----------------------------------------------------------------------
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
        bge       L111
...
_MakeBcc:
	...
	.line	1
;----------------------------------------------------------------------
; 407 | void MakeBcc(UCHAR *pDat,int nLen)                                     
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	.line	5
;----------------------------------------------------------------------
; 411 | pDat[nLen] = pDat[nLen+1] = 0x00;                                      
;----------------------------------------------------------------------
        ldiu      *-fp(3),ar0
	...
	.line	7
;----------------------------------------------------------------------
; 413 | for(i=0;i<nLen;i++)     pDat[nLen + (i&0x01)] ^= pDat[i];              
;----------------------------------------------------------------------
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
        bge       L116
...
_IsBccOk:
	...
	.line	1
;----------------------------------------------------------------------
; 420 | int IsBccOk(UCHAR *pDat,int nLen)                                      
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      3,sp
...
L120:        
	.line	13
;----------------------------------------------------------------------
; 432 | if(nBCC[0] != WORD_L(pDat[nLen]) || nBCC[1] != WORD_L(pDat[nLen+1]))   
;----------------------------------------------------------------------
        ldiu      fp,ar0
        ldiu      255,r0
        ldiu      *-fp(2),ir0
...
_crc16_ccitt:
	...
	.line	1
;----------------------------------------------------------------------
; 446 | WORD crc16_ccitt(UCHAR *buf, int len)                                  
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 450 | for( counter = 0; counter < len; counter++)                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
...
L128:        
	.line	6
;----------------------------------------------------------------------
; 451 | crc = (crc<<8) ^ crc16tab[((crc>>8)^buf[counter])&0x00FF];             
;----------------------------------------------------------------------
        ldiu      *+fp(2),r0
        ldiu      *-fp(2),ir0
        ldiu      *+fp(1),ar0
...
_gm_SysTimeToRtc:
	...
	.line	1
;----------------------------------------------------------------------
; 459 | BOOL gm_SysTimeToRtc(DATE_TIME_PTR pTime,UINT nSysTime)                
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      19,sp
...
;----------------------------------------------------------------------
	.line	4
;----------------------------------------------------------------------
; 462 | int days[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };       
; 463 | int nTemp;                                                             
;----------------------------------------------------------------------
        ldiu      @CL24,ar1
        ldiu      fp,ar0
...
||      ldi       *ar1++(1),r0
	...
	.line	6
;----------------------------------------------------------------------
; 464 | unsigned int nDays,nTotalSecond = nSysTime,nFrontSec,nRearSec;         
;----------------------------------------------------------------------
        ldiu      *-fp(3),r0
        sti       r0,*+fp(16)
	.line	7
;----------------------------------------------------------------------
; 465 | int nBaseYear = 2000;                                                  
;----------------------------------------------------------------------
        ldiu      2000,r0
        sti       r0,*+fp(19)
	.line	9
...
;----------------------------------------------------------------------
	...
	.line	10
;----------------------------------------------------------------------
; 468 | nRearSec = nFrontSec = 0;                                              
;----------------------------------------------------------------------
        sti       r0,*+fp(17)
        sti       r0,*+fp(18)
	.line	11
;----------------------------------------------------------------------
; 469 | for(nTemp = nBaseYear; nTemp <= 2245 ;nTemp++)                         
;----------------------------------------------------------------------
        ldiu      *+fp(19),r0
        sti       r0,*+fp(14)
        cmpi      2245,r0
...
;----------------------------------------------------------------------
	...
	.line	14
;----------------------------------------------------------------------
; 472 | if((!(nTemp % 4) && (nTemp % 100)) || !(nTemp % 400)) nDays += 1;      
;----------------------------------------------------------------------
        ldiu      *+fp(14),r0
        ldiu      r0,r1
        ash       -1,r1
...
L137:        
	.line	15
;----------------------------------------------------------------------
; 473 | nRearSec = nFrontSec;                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(17),r0
        sti       r0,*+fp(18)
	.line	16
;----------------------------------------------------------------------
; 474 | nFrontSec += nDays*24*60*60;                                           
;----------------------------------------------------------------------
        ldiu      *+fp(15),r0
        mpyi      24,r0
        mpyi      60,r0
	...
	.line	17
;----------------------------------------------------------------------
; 475 | if(ISRANGE_LOW(nTotalSecond,nRearSec,nFrontSec))                       
;----------------------------------------------------------------------
        ldiu      *+fp(16),r0
        cmpi      *+fp(18),r0
        blo       L140
...
;*      Branch Occurs to L140 
	.line	19
;----------------------------------------------------------------------
; 477 | pTime->year = BIN2BCD(nTemp-nBaseYear);                                
;----------------------------------------------------------------------
        ldiu      *+fp(19),r0
        subri     *+fp(14),r0
        ldiu      10,r1
...
;----------------------------------------------------------------------
	...
	.line	30
;----------------------------------------------------------------------
; 488 | nRearSec = nFrontSec = 0;                                              
;----------------------------------------------------------------------
        sti       r0,*+fp(17)
        sti       r0,*+fp(18)
	.line	31
;----------------------------------------------------------------------
; 489 | for(nTemp=0;nTemp<11;nTemp++)                                          
;----------------------------------------------------------------------
        sti       r0,*+fp(14)
        cmpi      11,r0
        bge       L148
...
;----------------------------------------------------------------------
	...
	.line	34
;----------------------------------------------------------------------
; 492 | nRearSec = nFrontSec;                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(17),r0
        sti       r0,*+fp(18)
	.line	35
;----------------------------------------------------------------------
; 493 | nFrontSec = nDays*24*60*60;                                            
;----------------------------------------------------------------------
        ldiu      *+fp(15),r0
        mpyi      24,r0
        mpyi      60,r0
	...
	.line	36
;----------------------------------------------------------------------
; 494 | if(ISRANGE_LOW(nTotalSecond,nRearSec,nFrontSec))                       
;----------------------------------------------------------------------
        ldiu      *+fp(16),r0
        cmpi      *+fp(18),r0
        blo       L147
...
;*      Branch Occurs to L147 
	.line	38
;----------------------------------------------------------------------
; 496 | pTime->month = BIN2BCD(nTemp+1);                                       
;----------------------------------------------------------------------
        ldiu      1,r0
        addi      *+fp(14),r0
        ldiu      10,r1
...
;*      Branch Occurs to L153 
	.line	47
;----------------------------------------------------------------------
; 505 | nTemp = nTotalSecond/(24*60*60);                                       
;----------------------------------------------------------------------
L151:        
        call      DIV_U30
                                        ; Call Occurs
...
;----------------------------------------------------------------------
	...
	.line	56
;----------------------------------------------------------------------
; 514 | pTime->minute = BIN2BCD(nTemp);                                        
;----------------------------------------------------------------------
        ldiu      10,r1
        call      DIV_I30
                                        ; Call Occurs
...
;----------------------------------------------------------------------
	...
	.line	59
;----------------------------------------------------------------------
; 517 | pTime->second = BIN2BCD(nTotalSecond);                                 
;----------------------------------------------------------------------
        ldiu      10,r1
        call      DIV_U30
                                        ; Call Occurs
...
_c_int10:
	...
	.line	1
;----------------------------------------------------------------------
; 556 | void c_int10(void)                                                     
; 558 | static int nRunLedCnt=0;                                               
;----------------------------------------------------------------------
        push      st
        push      fp
        ldiu      sp,fp
...
;----------------------------------------------------------------------
	...
	.line	14
;----------------------------------------------------------------------
; 569 | RUN_LED(nRunLedCnt&0x20 ? 1 : 0);                                      
;----------------------------------------------------------------------
        ldiu      32,r0
