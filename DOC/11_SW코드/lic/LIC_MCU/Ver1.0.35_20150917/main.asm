...
;* Date/Time created: Tue Nov 17 16:05:23 2015                                *
;******************************************************************************
...
_FirmwareVersionStrCmp:
	...
	.line	1
;----------------------------------------------------------------------
; 363 | BOOL FirmwareVersionStrCmp(char *pObj,char *pSrc,int nLen)             
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 367 | for(i=0;i<nLen;i++) if(pObj[i] != pSrc[i]) {bFlag = FALSE; break;}     
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        cmpi      *-fp(4),r0
...
_GetFirmwareVersion:
	...
	.line	1
;----------------------------------------------------------------------
; 376 | int GetFirmwareVersion(UCHAR nVerBuildData)                            
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      3,sp
...
L100:        
	.line	9
;----------------------------------------------------------------------
; 384 | pVersionString = (char *)m_pBoardIdInfo[i];                            
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0
        ldiu      @CL19,ar0
        ldi       *+ar0(ir0),r0
	...
	.line	10
;----------------------------------------------------------------------
; 385 | if(pVersionString == NULL) break;                                      
;----------------------------------------------------------------------
        beq       L109
;*      Branch Occurs to L109 
	.line	12
;----------------------------------------------------------------------
; 387 | if(WORD_L(nVerBuildData) == 1)                                         
;----------------------------------------------------------------------
        ldiu      255,r0
        and       *-fp(2),r0
        cmpi      1,r0
...
;*      Branch Occurs to L105 
	.line	14
;----------------------------------------------------------------------
; 389 | if(FirmwareVersionStrCmp("Version",pVersionString,7))                  
;----------------------------------------------------------------------
        ldiu      7,r0
        push      r0
        ldiu      *+fp(3),r1
...
;*      Branch Occurs to L108 
	.line	16
;----------------------------------------------------------------------
; 391 | nVer = ConvAsc2Dec(pVersionString[8])*1000+ConvAsc2Dec(pVersionString[1
...
L105:        
	.line	20
;----------------------------------------------------------------------
; 395 | else if(WORD_L(nVerBuildData) == 2)                                    
;----------------------------------------------------------------------
        ldiu      255,r0
        and       *-fp(2),r0
        cmpi      2,r0
...
;*      Branch Occurs to L108 
	.line	22
;----------------------------------------------------------------------
; 397 | if(FirmwareVersionStrCmp("Date",pVersionString,4))                     
;----------------------------------------------------------------------
        ldiu      4,r0
        ldiu      @CL21,r1
        push      r0
...
;*      Branch Occurs to L108 
	.line	24
;----------------------------------------------------------------------
; 399 | nVer = ConvAsc2Dec(pVersionString[7])*100000+ConvAsc2Dec(pVersionString
;     | [8])*10000+ConvAsc2Dec(pVersionString[10])*1000+                       
; 400 |        ConvAsc2Dec(pVersionString[11])*100+ConvAsc2Dec(pVersionString[1
...
_Make1ByteBcc:
	...
	.line	1
;----------------------------------------------------------------------
; 412 | UCHAR Make1ByteBcc(UCHAR *pDat,int nLen)                               
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	.line	4
;----------------------------------------------------------------------
; 415 | UCHAR btBcc = 0x00;                                                    
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(2)
	.line	5
;----------------------------------------------------------------------
; 416 | for(i=0;i<nLen;i++)     btBcc ^= pDat[i];                              
;----------------------------------------------------------------------
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
        bge       L114
...
_MakeBcc:
	...
	.line	1
;----------------------------------------------------------------------
; 423 | void MakeBcc(UCHAR *pDat,int nLen)                                     
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      1,sp
...
;----------------------------------------------------------------------
	.line	5
;----------------------------------------------------------------------
; 427 | pDat[nLen] = pDat[nLen+1] = 0x00;                                      
;----------------------------------------------------------------------
        ldiu      *-fp(3),ar0
	...
	.line	7
;----------------------------------------------------------------------
; 429 | for(i=0;i<nLen;i++)     pDat[nLen + (i&0x01)] ^= pDat[i];              
;----------------------------------------------------------------------
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
        bge       L119
...
_IsBccOk:
	...
	.line	1
;----------------------------------------------------------------------
; 436 | int IsBccOk(UCHAR *pDat,int nLen)                                      
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      3,sp
...
L123:        
	.line	13
;----------------------------------------------------------------------
; 448 | if(nBCC[0] != WORD_L(pDat[nLen]) || nBCC[1] != WORD_L(pDat[nLen+1]))   
;----------------------------------------------------------------------
        ldiu      fp,ar0
        ldiu      255,r0
        ldiu      *-fp(2),ir0
...
_crc16_ccitt:
	...
	.line	1
;----------------------------------------------------------------------
; 462 | WORD crc16_ccitt(UCHAR *buf, int len)                                  
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      2,sp
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 466 | for( counter = 0; counter < len; counter++)                            
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        cmpi      *-fp(3),r0
...
L131:        
	.line	6
;----------------------------------------------------------------------
; 467 | crc = (crc<<8) ^ crc16tab[((crc>>8)^buf[counter])&0x00FF];             
;----------------------------------------------------------------------
        ldiu      *+fp(2),r0
        ldiu      *-fp(2),ir0
        ldiu      *+fp(1),ar0
...
_gm_SysTimeToRtc:
	...
	.line	1
;----------------------------------------------------------------------
; 475 | BOOL gm_SysTimeToRtc(DATE_TIME_PTR pTime,UINT nSysTime)                
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      19,sp
...
;----------------------------------------------------------------------
	.line	4
;----------------------------------------------------------------------
; 478 | int days[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };       
; 479 | int nTemp;                                                             
;----------------------------------------------------------------------
        ldiu      @CL24,ar1
        ldiu      fp,ar0
...
||      ldi       *ar1++(1),r0
	...
	.line	6
;----------------------------------------------------------------------
; 480 | unsigned int nDays,nTotalSecond = nSysTime,nFrontSec,nRearSec;         
;----------------------------------------------------------------------
        ldiu      *-fp(3),r0
        sti       r0,*+fp(16)
	.line	7
;----------------------------------------------------------------------
; 481 | int nBaseYear = 2000;                                                  
;----------------------------------------------------------------------
        ldiu      2000,r0
        sti       r0,*+fp(19)
	.line	9
...
;----------------------------------------------------------------------
	...
	.line	10
;----------------------------------------------------------------------
; 484 | nRearSec = nFrontSec = 0;                                              
;----------------------------------------------------------------------
        sti       r0,*+fp(17)
        sti       r0,*+fp(18)
	.line	11
;----------------------------------------------------------------------
; 485 | for(nTemp = nBaseYear; nTemp <= 2245 ;nTemp++)                         
;----------------------------------------------------------------------
        ldiu      *+fp(19),r0
        sti       r0,*+fp(14)
        cmpi      2245,r0
...
;----------------------------------------------------------------------
	...
	.line	14
;----------------------------------------------------------------------
; 488 | if((!(nTemp % 4) && (nTemp % 100)) || !(nTemp % 400)) nDays += 1;      
;----------------------------------------------------------------------
        ldiu      *+fp(14),r0
        ldiu      r0,r1
        ash       -1,r1
...
L140:        
	.line	15
;----------------------------------------------------------------------
; 489 | nRearSec = nFrontSec;                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(17),r0
        sti       r0,*+fp(18)
	.line	16
;----------------------------------------------------------------------
; 490 | nFrontSec += nDays*24*60*60;                                           
;----------------------------------------------------------------------
        ldiu      *+fp(15),r0
        mpyi      24,r0
        mpyi      60,r0
	...
	.line	17
;----------------------------------------------------------------------
; 491 | if(ISRANGE_LOW(nTotalSecond,nRearSec,nFrontSec))                       
;----------------------------------------------------------------------
        ldiu      *+fp(16),r0
        cmpi      *+fp(18),r0
        blo       L143
...
;*      Branch Occurs to L143 
	.line	19
;----------------------------------------------------------------------
; 493 | pTime->year = BIN2BCD(nTemp-nBaseYear);                                
;----------------------------------------------------------------------
        ldiu      *+fp(19),r0
        subri     *+fp(14),r0
        ldiu      10,r1
...
;----------------------------------------------------------------------
	...
	.line	30
;----------------------------------------------------------------------
; 504 | nRearSec = nFrontSec = 0;                                              
;----------------------------------------------------------------------
        sti       r0,*+fp(17)
        sti       r0,*+fp(18)
	.line	31
;----------------------------------------------------------------------
; 505 | for(nTemp=0;nTemp<11;nTemp++)                                          
;----------------------------------------------------------------------
        sti       r0,*+fp(14)
        cmpi      11,r0
        bge       L151
...
;----------------------------------------------------------------------
	...
	.line	34
;----------------------------------------------------------------------
; 508 | nRearSec = nFrontSec;                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(17),r0
        sti       r0,*+fp(18)
	.line	35
;----------------------------------------------------------------------
; 509 | nFrontSec = nDays*24*60*60;                                            
;----------------------------------------------------------------------
        ldiu      *+fp(15),r0
        mpyi      24,r0
        mpyi      60,r0
	...
	.line	36
;----------------------------------------------------------------------
; 510 | if(ISRANGE_LOW(nTotalSecond,nRearSec,nFrontSec))                       
;----------------------------------------------------------------------
        ldiu      *+fp(16),r0
        cmpi      *+fp(18),r0
        blo       L150
...
;*      Branch Occurs to L150 
	.line	38
;----------------------------------------------------------------------
; 512 | pTime->month = BIN2BCD(nTemp+1);                                       
;----------------------------------------------------------------------
        ldiu      1,r0
        addi      *+fp(14),r0
        ldiu      10,r1
...
;*      Branch Occurs to L156 
	.line	47
;----------------------------------------------------------------------
; 521 | nTemp = nTotalSecond/(24*60*60);                                       
;----------------------------------------------------------------------
L154:        
        call      DIV_U30
                                        ; Call Occurs
...
;----------------------------------------------------------------------
	...
	.line	56
;----------------------------------------------------------------------
; 530 | pTime->minute = BIN2BCD(nTemp);                                        
;----------------------------------------------------------------------
        ldiu      10,r1
        call      DIV_I30
                                        ; Call Occurs
...
;----------------------------------------------------------------------
	...
	.line	59
;----------------------------------------------------------------------
; 533 | pTime->second = BIN2BCD(nTotalSecond);                                 
;----------------------------------------------------------------------
        ldiu      10,r1
        call      DIV_U30
                                        ; Call Occurs
...
_c_int10:
	...
	.line	1
;----------------------------------------------------------------------
; 572 | void c_int10(void)                                                     
; 574 | static int nRunLedCnt=0;                                               
;----------------------------------------------------------------------
        push      st
        push      fp
        ldiu      sp,fp
...
;----------------------------------------------------------------------
	...
	.line	14
;----------------------------------------------------------------------
; 585 | RUN_LED(nRunLedCnt&0x20 ? 1 : 0);                                      
;----------------------------------------------------------------------
        ldiu      32,r0
