...
;* Date/Time created: Tue Nov 17 16:05:21 2015                                *
;******************************************************************************
...
;----------------------------------------------------------------------
	...
	.line	5
;----------------------------------------------------------------------
; 128 | m_Variable.m_TrainCofVal = 1;                                          
;----------------------------------------------------------------------
        ldiu      1,r0
...
;*   Regs Saved         : r4                                                  *
;*   Stack Frame        : Full (w/ debug)                                     *
;*   Total Frame Size   : 2 Call + 0 Parm + 439 Auto + 1 SOE = 442 words      *
;******************************************************************************
_user_WithPacRs485Loop:
	...
	.sym	_nMasterCarNum,405,12,1,32
	.sym	_sAdd1,406,12,1,32
	.sym	_sAdd2,407,12,1,32
	.sym	_sAdd3,408,12,1,32
	.sym	_pLnWkDp,409,24,1,32,.fake10
	.sym	_pPaSdrBuf,410,24,1,32,.fake13
	.sym	_lsLicSdBuf,411,8,1,768,.fake36
	.sym	_pPa_PaBuf,435,24,1,32,.fake34
	.sym	_pCcp_Pac,436,24,1,32,.fake35
	.sym	_pPac_Pac_Sta,437,24,1,32,.fake14
	.sym	_pCommStatus,438,24,1,32,.fake49
	.sym	_pCommStatus_Lic,439,24,1,32,.fake57
	.line	1
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
	...
	.line	10
;----------------------------------------------------------------------
; 820 | UCHAR nMasterCarNum = 0;                                               
; 822 | static int nRxPos = 0;                                                 
; 823 | static UCHAR nOldUart3RxOneByteGapDelayTime = 0;                       
; 824 | static UCHAR btRx3chBuf[300];                                          
; 826 | static UCHAR sCarNumBCD_H = 0;                                         
; 827 | static UCHAR sCarNumBCD_L = 0;                                         
;----------------------------------------------------------------------
        ldiu      405,ir0
        sti       r0,*+fp(ir0)
	.line	20
...
;----------------------------------------------------------------------
	...
	.line	25
;----------------------------------------------------------------------
; 835 | PLNWKDP pLnWkDp = (LNWKDP *)DPRAM_BASE;                                
; 837 | PPACSDR pPaSdrBuf;                                                     
; 838 | LICSD lsLicSdBuf;                                                      
; 839 | PPAC_PAC pPa_PaBuf;                                                    
; 841 | PCCP_PAC pCcp_Pac;                                                     
; 843 | PCRA_STATION pPac_Pac_Sta;                                             
; 845 | PCOMMSTATUS_PA pCommStatus;                                            
; 846 | PCOMMSTATUS_LIC pCommStatus_Lic;                                       
; 849 | // ¼ö½Å                                                                
;----------------------------------------------------------------------
        ldiu      409,ir0
        ldiu      @CL75,r0
	...
	.line	40
;----------------------------------------------------------------------
; 850 | nRxLen = user_PacRx(btBuf,128);                                        
;----------------------------------------------------------------------
        ldiu      128,r1
        ldiu      fp,r0
        push      r1
...
;*      Branch Occurs to L212 
	.line	43
;----------------------------------------------------------------------
; 853 | if(!m_Variable.m_nUart3RxOneByteGapDelayTime) nRxPos = 0;              
;----------------------------------------------------------------------
        ldi       @_m_Variable+290,r0
        bne       L210
...
L210:        
	.line	44
;----------------------------------------------------------------------
; 854 | m_Variable.m_nUart3RxOneByteGapDelayTime = 3;                          
;----------------------------------------------------------------------
        ldiu      3,r0
        sti       r0,@_m_Variable+290
	.line	46
;----------------------------------------------------------------------
; 856 | if(nRxPos + nRxLen < 300)                                              
;----------------------------------------------------------------------
        ldiu      *+fp(2),r0
        addi      @_nRxPos$9+0,r0
        cmpi      300,r0
...
;*      Branch Occurs to L212 
	.line	48
;----------------------------------------------------------------------
; 858 | memcpy(&btRx3chBuf[nRxPos],btBuf,nRxLen);                              
;----------------------------------------------------------------------
...
L212:        
	.line	54
;----------------------------------------------------------------------
; 864 | if(nOldUart3RxOneByteGapDelayTime && !m_Variable.m_nUart3RxOneByteGapDe
;     | layTime)                                                               
;----------------------------------------------------------------------
...
;*      Branch Occurs to L266 
	.line	59
;----------------------------------------------------------------------
; 869 | pPaSdrBuf = (PACSDR *)btRx3chBuf;                                      
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      @CL76,r0
        sti       r0,*+fp(ir0)
	.line	61
;----------------------------------------------------------------------
; 871 | pPa_PaBuf = (PAC_PAC *)btRx3chBuf;                                     
;----------------------------------------------------------------------
        ldiu      435,ir0
        sti       r0,*+fp(ir0)
	.line	64
;----------------------------------------------------------------------
; 874 | pCcp_Pac = (CCP_PAC *)btRx3chBuf;                                      
; 876 | //PAC -> LIC ºÎºÐ ÇÁ·ÎÅäÄÝÀ» ºÐ¸® ÇÑ´Ù.                                
; 877 | if(                                                                    
;----------------------------------------------------------------------
        ldiu      436,ir0
        sti       r0,*+fp(ir0)
	.line	68
;----------------------------------------------------------------------
; 878 | WORD_L(pPaSdrBuf->phHdBuf.btSoh) == SOH &&                             
; 879 | WORD_L(pPaSdrBuf->btEot) == EOT &&                                     
; 880 | sizeof(PACSDR) == nRxPos &&                                            
; 881 | (TWOBYTE_ASC2HEX(pPaSdrBuf->phHdBuf.chDestDev) == m_Variable.m_chCarKin
;     | dNum)&&                                                                
; 883 | ConvAsc2Hex(pPaSdrBuf->nCRC[0]) == BYTE_H(WORD_H(crc16_ccitt(&pPaSdrBuf
;     | ->phHdBuf.chSrcDev[0],nRxPos-6)))&&                                    
; 884 | ConvAsc2Hex(pPaSdrBuf->nCRC[1]) == BYTE_L(WORD_H(crc16_ccitt(&pPaSdrBuf
;     | ->phHdBuf.chSrcDev[0],nRxPos-6)))&&                                    
; 885 | ConvAsc2Hex(pPaSdrBuf->nCRC[2]) == BYTE_H(WORD_L(crc16_ccitt(&pPaSdrBuf
;     | ->phHdBuf.chSrcDev[0],nRxPos-6)))&&                                    
; 886 | ConvAsc2Hex(pPaSdrBuf->nCRC[3]) == BYTE_L(WORD_L(crc16_ccitt(&pPaSdrBuf
;     | ->phHdBuf.chSrcDev[0],nRxPos-6))))                                     
; 888 | switch(TWOBYTE_ASC2HEX(pPaSdrBuf->phHdBuf.chCmdCode))                  
; 890 | case REQ_CMD: //PAC -> LIC                                             
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      255,r0
...
;*      Branch Occurs to L228 
	.line	82
;----------------------------------------------------------------------
; 892 | lsLicSdBuf.phHdBuf.btSoh = SOH;                                        
;----------------------------------------------------------------------
L225:        
        sti       r0,*+fp(ir0)
	.line	83
;----------------------------------------------------------------------
; 893 | lsLicSdBuf.phHdBuf.chSrcDev[0] = ConvHex2Asc(BYTE_H(m_Variable.m_chCarK
;     | indNum));                                                              
;----------------------------------------------------------------------
	...
	.line	84
;----------------------------------------------------------------------
; 894 | lsLicSdBuf.phHdBuf.chSrcDev[1] = ConvHex2Asc(BYTE_L(m_Variable.m_chCarK
;     | indNum));                                                              
;----------------------------------------------------------------------
        ldiu      15,r0
	...
	.line	85
;----------------------------------------------------------------------
; 895 | lsLicSdBuf.phHdBuf.chDestDev[0] = pPaSdrBuf->phHdBuf.chSrcDev[0];      
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      414,ir0
	...
	.line	86
;----------------------------------------------------------------------
; 896 | lsLicSdBuf.phHdBuf.chDestDev[1] = pPaSdrBuf->phHdBuf.chSrcDev[1];      
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      415,ir0
	...
	.line	87
;----------------------------------------------------------------------
; 897 | lsLicSdBuf.phHdBuf.chMsgCnt[0] = pPaSdrBuf->phHdBuf.chMsgCnt[0];       
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      416,ir0
	...
	.line	88
;----------------------------------------------------------------------
; 898 | lsLicSdBuf.phHdBuf.chMsgCnt[1] = pPaSdrBuf->phHdBuf.chMsgCnt[1];       
;----------------------------------------------------------------------
        ldiu      410,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      417,ir0
	...
	.line	89
;----------------------------------------------------------------------
; 899 | lsLicSdBuf.phHdBuf.chCmdCode[0] = ConvHex2Asc(BYTE_H(RPY_CMD));        
;----------------------------------------------------------------------
        ldiu      1,r0
        push      r0
        call      _ConvHex2Asc
	...
	.line	90
;----------------------------------------------------------------------
; 900 | lsLicSdBuf.phHdBuf.chCmdCode[1] = ConvHex2Asc(BYTE_L(RPY_CMD));        
;----------------------------------------------------------------------
        ldiu      1,r0
        push      r0
        call      _ConvHex2Asc
	...
	.line	91
;----------------------------------------------------------------------
; 901 | lsLicSdBuf.phHdBuf.chDataLen[0] = ConvHex2Asc(BYTE_H(((sizeof(LICSD)-16
;     | )/2)));                                                                
;----------------------------------------------------------------------
        ldiu      0,r0
        push      r0
	...
	.line	92
;----------------------------------------------------------------------
; 902 | lsLicSdBuf.phHdBuf.chDataLen[1] = ConvHex2Asc(BYTE_L(((sizeof(LICSD)-16
;     | )/2)));                                                                
;----------------------------------------------------------------------
        ldiu      4,r0
        push      r0
	...
	.line	94
;----------------------------------------------------------------------
; 904 | lsLicSdBuf.DATA1.BIT.All_Doors_Closed = m_Variable.m_btCttSignalA.BIT.A
;     | ll_Doors_Closed;                                                       
;----------------------------------------------------------------------
        ldiu      8,r0
        and       @_m_Variable+425,r0
	...
	.line	95
;----------------------------------------------------------------------
; 905 | lsLicSdBuf.DATA1.BIT.EP_Mode = m_Variable.m_btCttSignalA.BIT.EP_Mode;  
;----------------------------------------------------------------------
        ldiu      4,r0
        ldiu      *+fp(ir0),r1
        and       @_m_Variable+425,r0
	...
	.line	96
;----------------------------------------------------------------------
; 906 | lsLicSdBuf.DATA1.BIT.Traction = m_Variable.m_btCttSignalA.BIT.Traction;
;----------------------------------------------------------------------
        ldiu      2,r0
        and       @_m_Variable+425,r0
        lsh       -1,r0
	...
	.line	97
;----------------------------------------------------------------------
; 907 | lsLicSdBuf.DATA1.BIT.Atcive_Cab = m_Variable.m_btCttSignalA.BIT.Atcive_
;     | Cab;                                                                   
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *+fp(ir0),r1
	...
	.line	99
;----------------------------------------------------------------------
; 909 | lsLicSdBuf.DATA2.BIT.CI_Fault = m_Variable.m_bCiFaultFlag;             
;----------------------------------------------------------------------
        ldiu      422,ir0
        ldiu      1,r0
        ldiu      *+fp(ir0),r1
	...
	.line	100
;----------------------------------------------------------------------
; 910 | lsLicSdBuf.DATA2.BIT.DST = 0;                                          
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),r0
        andn      8,r0
        sti       r0,*+fp(ir1)
	.line	102
;----------------------------------------------------------------------
; 912 | lsLicSdBuf.DATA1.BYTE = ConvHex2Asc(BYTE_L(lsLicSdBuf.DATA1.BYTE));    
;----------------------------------------------------------------------
        ldiu      423,ir0
        ldiu      15,r0
        and3      r0,*+fp(ir0),r0
	...
	.line	104
;----------------------------------------------------------------------
; 914 | lsLicSdBuf.DATA2.BYTE = ConvHex2Asc(BYTE_L(lsLicSdBuf.DATA2.BYTE));    
;----------------------------------------------------------------------
        ldiu      422,ir0
        ldiu      15,r0
        and3      r0,*+fp(ir0),r0
	...
	.line	106
;----------------------------------------------------------------------
; 916 | lsLicSdBuf.CI_Index_Num[0] = ConvHex2Asc(BYTE_H(m_Variable.m_nCarPos));
;----------------------------------------------------------------------
	...
	.line	107
;----------------------------------------------------------------------
; 917 | lsLicSdBuf.CI_Index_Num[1] = ConvHex2Asc(BYTE_L(m_Variable.m_nCarPos));
;----------------------------------------------------------------------
        ldiu      15,r0
	...
	.line	109
;----------------------------------------------------------------------
; 919 | lsLicSdBuf.chCarn[0][0] = ConvHex2Asc(BYTE_H(WORD_H(m_Variable.m_nCarNo
;     | ))); // Car Number(-.¹éÀÚ¸®)                                           
;----------------------------------------------------------------------
        ldiu      @_m_Variable+341,r0
        lsh       -8,r0
	...
	.line	110
;----------------------------------------------------------------------
; 920 | lsLicSdBuf.chCarn[0][1] = ConvHex2Asc(BYTE_L(WORD_H(m_Variable.m_nCarNo
;     | )));                                                                   
;----------------------------------------------------------------------
        ldiu      @_m_Variable+341,r0
        lsh       -8,r0
	...
	.line	111
;----------------------------------------------------------------------
; 921 | lsLicSdBuf.chCarn[1][0] = ConvHex2Asc(BYTE_H(m_Variable.m_nCarNo)); //
;     | Car Number(½ÊÀÚ¸®,ÀÏÀÚ¸®)                                              
;----------------------------------------------------------------------
        ldiu      @_m_Variable+341,r0
        lsh       -4,r0
	...
	.line	112
;----------------------------------------------------------------------
; 922 | lsLicSdBuf.chCarn[1][1] = ConvHex2Asc(BYTE_L(m_Variable.m_nCarNo));    
;----------------------------------------------------------------------
        ldiu      15,r0
        and       @_m_Variable+341,r0
        push      r0
	...
	.line	114
;----------------------------------------------------------------------
; 924 | btTmp = crc16_ccitt(&lsLicSdBuf.phHdBuf.chSrcDev[0],sizeof(LICSD)-6);  
;----------------------------------------------------------------------
        ldiu      18,r1
        ldiu      412,r0
        push      r1
	...
	.line	115
;----------------------------------------------------------------------
; 925 | lsLicSdBuf.nCRC[0] = ConvHex2Asc(BYTE_H(WORD_H(btTmp)));               
;----------------------------------------------------------------------
        lsh       -8,r0
        and       255,r0
        lsh       -4,r0
	...
	.line	116
;----------------------------------------------------------------------
; 926 | lsLicSdBuf.nCRC[1] = ConvHex2Asc(BYTE_L(WORD_H(btTmp)));               
;----------------------------------------------------------------------
        ldiu      *+fp(4),r0
        lsh       -8,r0
        and       15,r0
	...
	.line	117
;----------------------------------------------------------------------
; 927 | lsLicSdBuf.nCRC[2] = ConvHex2Asc(BYTE_H(WORD_L(btTmp)));               
;----------------------------------------------------------------------
        ldiu      255,r0
        and       *+fp(4),r0
        lsh       -4,r0
	...
	.line	118
;----------------------------------------------------------------------
; 928 | lsLicSdBuf.nCRC[3] = ConvHex2Asc(BYTE_L(WORD_L(btTmp)));               
;----------------------------------------------------------------------
        ldiu      15,r0
        and       *+fp(4),r0
        push      r0
...
;----------------------------------------------------------------------
	...
	.line	121
;----------------------------------------------------------------------
; 931 | user_PacTx((UCHAR *)&lsLicSdBuf,sizeof(LICSD));                        
;----------------------------------------------------------------------
        ldiu      24,r0
        push      r0
        ldiu      411,r0
...
L231:        
	.line	130
;----------------------------------------------------------------------
; 940 | if(WORD_L(pPa_PaBuf->phHdBuf.btSoh) == SOH &&  // PAC <-> PAC ¿¡¼­ »óÅÂ
;     |  Á¤º¸¸¦ ÃßÃâÇÑ´Ù.                                                      
; 941 |             WORD_L(pPa_PaBuf->btEot) == EOT &&                         
; 942 |             sizeof(PAC_PAC) == nRxPos &&                               
; 943 |                 //(TWOBYTE_ASC2HEX(pPa_PaBuf->phHdBuf.chSrcDev) == PAC_
;     | DEV_NO) &&                                                             
; 944 |             //(TWOBYTE_ASC2HEX(pPa_PaBuf->phHdBuf.chDestDev) == PAC_DEV
;     | _NO) &&                                                                
; 946 |             ConvAsc2Hex(pPa_PaBuf->nCRC[0]) == BYTE_H(WORD_H(crc16_ccit
;     | t(&pPa_PaBuf->phHdBuf.chSrcDev[0],nRxPos-6)))&&                        
; 947 |                 ConvAsc2Hex(pPa_PaBuf->nCRC[1]) == BYTE_L(WORD_H(crc16_
;     | ccitt(&pPa_PaBuf->phHdBuf.chSrcDev[0],nRxPos-6)))&&                    
; 948 |                 ConvAsc2Hex(pPa_PaBuf->nCRC[2]) == BYTE_H(WORD_L(crc16_
;     | ccitt(&pPa_PaBuf->phHdBuf.chSrcDev[0],nRxPos-6)))&&                    
; 949 |                 ConvAsc2Hex(pPa_PaBuf->nCRC[3]) == BYTE_L(WORD_L(crc16_
;     | ccitt(&pPa_PaBuf->phHdBuf.chSrcDev[0],nRxPos-6)))                      
; 950 |             )                                                          
;----------------------------------------------------------------------
        ldiu      435,ir0
...
;*      Branch Occurs to L252 
	.line	142
;----------------------------------------------------------------------
; 952 | memset(m_Variable.m_btCommSt,0x00,sizeof(m_Variable.m_btCommSt));      
;----------------------------------------------------------------------
        ldiu      8,r2
        ldiu      0,r0
        ldiu      @CL3,r1
	...
	.line	144
;----------------------------------------------------------------------
; 954 | pCommStatus_Lic = (COMMSTATUS_LIC *)m_Variable.m_btCommSt;             
;----------------------------------------------------------------------
        ldiu      439,ir0
        ldiu      @CL3,r0
        sti       r0,*+fp(ir0)
	.line	146
;----------------------------------------------------------------------
; 956 | m_Variable.m_TrainCofVal = TWOBYTE_ASC2HEX(pPa_PaBuf->sCCI); //// Â÷·®
;     | Áß·Ã »óÅÂ Á¤º¸¸¦ Àü´Þ.                                                 
;----------------------------------------------------------------------
        ldiu      435,ir0
        ldiu      *+fp(ir0),ar0
	...
	.line	148
;----------------------------------------------------------------------
; 958 | if(m_Variable.m_TrainCofVal == 0x10 || m_Variable.m_TrainCofVal == 0x20
;     | )                                                                      
;----------------------------------------------------------------------
        cmpi      16,r0
        beq       L240
...
L240:        
	.line	150
;----------------------------------------------------------------------
; 960 | m_Variable.m_TrainCofVal = 0x01;                                       
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,@_m_Variable+337
L241:        
	.line	153
;----------------------------------------------------------------------
; 963 | m_Variable.m_btCiFaultVal = TWOBYTE_ASC2HEX(pPa_PaBuf->sCI_Fault); //CI
;     |  °íÀå Á¤º¸.                                                            
;----------------------------------------------------------------------
        ldiu      435,ir0
        ldiu      *+fp(ir0),ar0
	...
	.line	156
;----------------------------------------------------------------------
; 966 | FunConvAscHex((char *)&pPa_PaBuf->phCRA_Sta[0][9][0],&sCarNumBCD_H,1); 
;----------------------------------------------------------------------
        ldiu      435,ir0
        ldiu      169,r2
        ldiu      @CL77,r1
	...
	.line	157
;----------------------------------------------------------------------
; 967 | FunConvAscHex((char *)&pPa_PaBuf->phCRA_Sta[0][10][0],&sCarNumBCD_L,1);
;----------------------------------------------------------------------
        ldiu      171,r2
        ldiu      435,ir0
        ldiu      1,r0
	...
	.line	159
;----------------------------------------------------------------------
; 969 | nMasterCarNum = MAKE_WORD(sCarNumBCD_H,sCarNumBCD_L);                  
;----------------------------------------------------------------------
        ldiu      255,r1
...
;----------------------------------------------------------------------
	...
	.line	163
;----------------------------------------------------------------------
; 973 | for(i=0;i<(WORD_L(m_Variable.m_TrainCofVal));i++)                      
;----------------------------------------------------------------------
        ldiu      0,r0
        sti       r0,*+fp(1)
        ldiu      255,r0
...
L242:        
	.line	167
;----------------------------------------------------------------------
; 977 | FunConvAscHex((char *)&pPa_PaBuf->phCRA_Sta[i][9][0],&sCarNumBCD_H,1); 
;----------------------------------------------------------------------
        ldiu      *+fp(1),r0
        ldiu      435,ir0
        ldiu      1,r2
	...
	.line	168
;----------------------------------------------------------------------
; 978 | FunConvAscHex((char *)&pPa_PaBuf->phCRA_Sta[i][10][0],&sCarNumBCD_L,1);
; 980 | //8·® ÀüÃ¼ ÇÁ·ÎÅäÄÝ¿¡¼­ ¿­¹øÀ» ÀÌ¿ëÇÏ¿© ÇöÀç ÀÚ½ÅÀÇ À§Ä¡¸¦ È®ÀÎ ÇÑ´Ù.  
;----------------------------------------------------------------------
        ldiu      435,ir0
        ldiu      *+fp(1),r1
        ldiu      1,r2
	...
	.line	172
;----------------------------------------------------------------------
; 982 | MyPrintf("CarNum : [%X].\r\n",MAKE_WORD(sCarNumBCD_H,sCarNumBCD_L));   
;----------------------------------------------------------------------
        ldiu      255,r2
        ldiu      @_sCarNumBCD_H$12+0,r0
        and       @_sCarNumBCD_L$13+0,r2
        ash       8,r0
        or3       r0,r2,r0
        and       @CL80,r0
        push      r0
	...
	.line	174
;----------------------------------------------------------------------
; 984 | if(BCD_BIN(DWORD_L(m_Variable.m_nCarNo)) == BCD_BIN(MAKE_WORD(sCarNumBC
;     | D_H,sCarNumBCD_L)) ||                                                  
; 985 |    BCD_BIN(DWORD_L(m_Variable.m_nCarNo)) == BCD_BIN(MAKE_WORD(sCarNumBC
;     | D_H,sCarNumBCD_L))+1 ||                                                
; 986 |    BCD_BIN(DWORD_L(m_Variable.m_nCarNo)) == BCD_BIN(MAKE_WORD(sCarNumBC
;     | D_H,sCarNumBCD_L))-1)                                                  
; 990 |         //ÀÚ½ÅÀÇ À§Ä¡¸¦ È®ÀÎ ÇÏ°í »óÅÂ °ªÀ» ÀÐ¾î ¿Â´Ù.                 
;----------------------------------------------------------------------
        ldiu      @_sCarNumBCD_H$12+0,r0
        ldiu      255,r1
        and       @_sCarNumBCD_L$13+0,r1
...
L245:        
	.line	181
;----------------------------------------------------------------------
; 991 | FunConvAscHex((char *)&pPa_PaBuf->phCRA_Sta[i][0][0],btTmpBuf,22);     
;----------------------------------------------------------------------
        ldiu      *+fp(1),r0
        ldiu      435,ir0
        ldiu      22,r2
	...
	.line	183
;----------------------------------------------------------------------
; 993 | pPac_Pac_Sta = (CRA_STATION *) btTmpBuf;                               
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      fp,r0
        addi      5,r0
	...
	.line	235
;----------------------------------------------------------------------
; 1045 | if(m_Variable.m_chCarKind == 'A')                                      
;----------------------------------------------------------------------
        ldiu      @_m_Variable+342,r0
        cmpi      65,r0
        bne       L248
;*      Branch Occurs to L248 
	.line	239
;----------------------------------------------------------------------
; 1049 | pCommStatus_Lic->BYTE_1.BIT.nCcp = pPac_Pac_Sta->CRA_2.BIT.sACCP1;     
;----------------------------------------------------------------------
        ldiu      439,ir1
        ldiu      8,r0
        ldiu      *+fp(ir0),ar1
	...
	.line	240
;----------------------------------------------------------------------
; 1050 | pCommStatus_Lic->BYTE_3.BIT.nCncs = pPac_Pac_Sta->CRA_3.BIT.sACNCS;    
; 1051 | //pCommStatus_Lic->BYTE_1.BIT.nGps = pPac_Pac_Sta->CRA_2.BIT.sAGPS;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      *+fp(ir1),ar0
        ldiu      1,r0
	...
	.line	242
;----------------------------------------------------------------------
; 1052 | pCommStatus_Lic->BYTE_1.BIT.nLic = pPac_Pac_Sta->CRA_3.BIT.sALIC;      
;----------------------------------------------------------------------
        ldiu      *+fp(ir1),ar0
        ldiu      4,r0
        ldiu      *ar0,r1
	...
	.line	243
;----------------------------------------------------------------------
; 1053 | pCommStatus_Lic->BYTE_1.BIT.nPac = pPac_Pac_Sta->CRA_3.BIT.sAPAC;      
;----------------------------------------------------------------------
        ldiu      8,r0
        ldiu      *+fp(ir0),ar1
        ldiu      *+fp(ir1),ar0
	...
	.line	244
;----------------------------------------------------------------------
; 1054 | pCommStatus_Lic->BYTE_1.BIT.nVoip = pPac_Pac_Sta->CRA_2.BIT.sAVOIP;    
;----------------------------------------------------------------------
        ldiu      4,r0
        ldiu      *+fp(ir0),ar1
        ldiu      *+fp(ir1),ar0
	...
	.line	245
;----------------------------------------------------------------------
; 1055 | pCommStatus_Lic->BYTE_1.BIT.nVtx = pPac_Pac_Sta->CRA_3.BIT.sAVTX;      
; 1056 | //pCommStatus_Lic->BYTE_1.BIT.nWlr = pPac_Pac_Sta->CRA_2.BIT.sAWLR;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      2,r0
        and       *+ar0(2),r0
	...
	.line	249
;----------------------------------------------------------------------
; 1059 | pCommStatus_Lic->BYTE_1.BIT.nFdi = pPac_Pac_Sta->CRA_4.BIT.sAFDI;      
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar1
        ldiu      4,r0
	...
	.line	250
;----------------------------------------------------------------------
; 1060 | pCommStatus_Lic->BYTE_2.BIT.nPii1 = pPac_Pac_Sta->CRA_4.BIT.sAPII1;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      2,r0
        and       *+ar1(3),r0
	...
	.line	251
;----------------------------------------------------------------------
; 1061 | pCommStatus_Lic->BYTE_2.BIT.nPii2 = pPac_Pac_Sta->CRA_4.BIT.sAPII2;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir1),ar0
        ldiu      *+fp(ir0),ar1
        ldiu      1,r0
	...
	.line	252
;----------------------------------------------------------------------
; 1062 | pCommStatus_Lic->BYTE_1.BIT.nSdi1 = pPac_Pac_Sta->CRA_5.BIT.sASDI1;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      8,r0
        ldiu      *+fp(ir1),ar0
	...
	.line	253
;----------------------------------------------------------------------
; 1063 | pCommStatus_Lic->BYTE_2.BIT.nSdi2 = pPac_Pac_Sta->CRA_5.BIT.sASDI2;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      4,r0
        and       *+ar1(4),r0
	...
	.line	254
;----------------------------------------------------------------------
; 1064 | pCommStatus_Lic->BYTE_2.BIT.nSdi3 = pPac_Pac_Sta->CRA_5.BIT.sASDI3;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      2,r0
        and       *+ar1(4),r0
	...
	.line	255
;----------------------------------------------------------------------
; 1065 | pCommStatus_Lic->BYTE_2.BIT.nSdi4 = pPac_Pac_Sta->CRA_5.BIT.sASDI4;    
;----------------------------------------------------------------------
        ldiu      1,r0
        ldiu      *+fp(ir1),ar0
        ldiu      *+fp(ir0),ar1
	...
	.line	257
;----------------------------------------------------------------------
; 1067 | pCommStatus_Lic->BYTE_2.BIT.nPid1_1 = pPac_Pac_Sta->CRA_6.BIT.sAPID1_1;
;----------------------------------------------------------------------
        ldiu      8,r0
        ldiu      *+fp(ir0),ar1
        ldiu      *+fp(ir1),ar0
	...
	.line	258
;----------------------------------------------------------------------
; 1068 | pCommStatus_Lic->BYTE_2.BIT.nPid1_2 = pPac_Pac_Sta->CRA_6.BIT.sAPID1_2;
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      4,r0
        and       *+ar1(5),r0
	...
	.line	259
;----------------------------------------------------------------------
; 1069 | pCommStatus_Lic->BYTE_2.BIT.nPid1_3 = pPac_Pac_Sta->CRA_6.BIT.sAPID1_3;
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      2,r0
        ldiu      439,ir0
	...
	.line	260
;----------------------------------------------------------------------
; 1070 | pCommStatus_Lic->BYTE_3.BIT.nPid1_4 = pPac_Pac_Sta->CRA_6.BIT.sAPID1_4;
;----------------------------------------------------------------------
        ldiu      *+fp(ir1),ar0
        ldiu      437,ir0
        ldiu      1,r0
	...
	.line	261
;----------------------------------------------------------------------
; 1071 | pCommStatus_Lic->BYTE_3.BIT.nPid2_1 = pPac_Pac_Sta->CRA_7.BIT.sAPID2_1;
; 1073 | //pCommStatus_Lic->BYTE_4.BIT.nDph = pPac_Pac_Sta->CRA_9.BIT.sADPH;    
; 1074 | //pCommStatus_Lic->BYTE_4.BIT.nDpo = pPac_Pac_Sta->CRA_9.BIT.sADPO;    
; 1075 | //pCommStatus_Lic->BYTE_4.BIT.nPei1 = pPac_Pac_Sta->CRA_7.BIT.sAPEI1;  
; 1076 | //pCommStatus_Lic->BYTE_4.BIT.nPei2 = pPac_Pac_Sta->CRA_7.BIT.sAPEI2;  
; 1080 | //pCommStatus_Lic->BYTE_1.BYTE = d_MDSFaulgTestBuf[0];                 
; 1081 | //pCommStatus_Lic->BYTE_2.BYTE = d_MDSFaulgTestBuf[1];                 
; 1082 | //pCommStatus_Lic->BYTE_3.BYTE = d_MDSFaulgTestBuf[2];                 
; 1083 | //pCommStatus_Lic->BYTE_4.BYTE = d_MDSFaulgTestBuf[3];                 
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      439,ir0
        ldiu      8,r0
...
L248:        
	.line	280
;----------------------------------------------------------------------
; 1090 | else if(m_Variable.m_chCarKind == 'B') //ÇÁ·ÎÅäÄÝ¿¡´Â MA ¿­Â÷ ¹øÈ£¸¸ ÀÖ
...
;*      Branch Occurs to L250 
	.line	312
;----------------------------------------------------------------------
; 1122 | pCommStatus_Lic->BYTE_1.BIT.nCcp = pPac_Pac_Sta->CRA_2.BIT.sBCCP1;     
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      439,ir1
        ldiu      128,r0
	...
	.line	313
;----------------------------------------------------------------------
; 1123 | pCommStatus_Lic->BYTE_3.BIT.nCncs = pPac_Pac_Sta->CRA_3.BIT.sBCNCS;    
; 1124 | //pCommStatus_Lic->BYTE_1.BIT.nGps pPac_Pac_Sta->CRA_2.BIT.sBGPS;      
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      *+fp(ir1),ar0
        ldiu      16,r0
	...
	.line	315
;----------------------------------------------------------------------
; 1125 | pCommStatus_Lic->BYTE_1.BIT.nLic = pPac_Pac_Sta->CRA_3.BIT.sBLIC;      
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      64,r0
        and       *+ar1(2),r0
	...
	.line	316
;----------------------------------------------------------------------
; 1126 | pCommStatus_Lic->BYTE_1.BIT.nPac = pPac_Pac_Sta->CRA_3.BIT.sBPAC;      
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      128,r0
        and       *+ar1(2),r0
	...
	.line	317
;----------------------------------------------------------------------
; 1127 | pCommStatus_Lic->BYTE_1.BIT.nVoip = pPac_Pac_Sta->CRA_2.BIT.sBVOIP;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      64,r0
        and       *+ar0(1),r0
	...
	.line	318
;----------------------------------------------------------------------
; 1128 | pCommStatus_Lic->BYTE_1.BIT.nVtx = pPac_Pac_Sta->CRA_3.BIT.sBVTX;      
; 1129 | //pCommStatus_Lic->BYTE_1.BIT.nWlr pPac_Pac_Sta->CRA_2.BIT.sBWLR;      
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      32,r0
	...
	.line	322
;----------------------------------------------------------------------
; 1132 | pCommStatus_Lic->BYTE_1.BIT.nFdi = pPac_Pac_Sta->CRA_4.BIT.sBFDI;      
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      64,r0
	...
	.line	323
;----------------------------------------------------------------------
; 1133 | pCommStatus_Lic->BYTE_2.BIT.nPii1 = pPac_Pac_Sta->CRA_4.BIT.sBPII1;    
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar1
        ldiu      32,r0
	...
	.line	324
;----------------------------------------------------------------------
; 1134 | pCommStatus_Lic->BYTE_2.BIT.nPii2 = pPac_Pac_Sta->CRA_4.BIT.sBPII2;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      16,r0
        and       *+ar1(3),r0
	...
	.line	325
;----------------------------------------------------------------------
; 1135 | pCommStatus_Lic->BYTE_1.BIT.nSdi1 = pPac_Pac_Sta->CRA_5.BIT.sBSDI1;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      128,r0
        and       *+ar1(4),r0
	...
	.line	326
;----------------------------------------------------------------------
; 1136 | pCommStatus_Lic->BYTE_2.BIT.nSdi2 = pPac_Pac_Sta->CRA_5.BIT.sBSDI2;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      64,r0
        and       *+ar1(4),r0
	...
	.line	327
;----------------------------------------------------------------------
; 1137 | pCommStatus_Lic->BYTE_2.BIT.nSdi3 = pPac_Pac_Sta->CRA_5.BIT.sBSDI3;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      32,r0
        and       *+ar1(4),r0
	...
	.line	328
;----------------------------------------------------------------------
; 1138 | pCommStatus_Lic->BYTE_2.BIT.nSdi4 = pPac_Pac_Sta->CRA_5.BIT.sBSDI4;    
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      16,r0
        and       *+ar1(4),r0
	...
	.line	330
;----------------------------------------------------------------------
; 1140 | pCommStatus_Lic->BYTE_2.BIT.nPid1_1 = pPac_Pac_Sta->CRA_6.BIT.sBPID1_1;
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar1
        ldiu      128,r0
        ldiu      *+fp(ir1),ar0
	...
	.line	331
;----------------------------------------------------------------------
; 1141 | pCommStatus_Lic->BYTE_2.BIT.nPid1_2 = pPac_Pac_Sta->CRA_6.BIT.sBPID1_2;
;----------------------------------------------------------------------
        ldiu      *+fp(ir0),ar0
        ldiu      64,r0
        ldiu      439,ir0
	...
	.line	332
;----------------------------------------------------------------------
; 1142 | pCommStatus_Lic->BYTE_2.BIT.nPid1_3 = pPac_Pac_Sta->CRA_6.BIT.sBPID1_3;
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar1
        ldiu      32,r0
	...
	.line	333
;----------------------------------------------------------------------
; 1143 | pCommStatus_Lic->BYTE_3.BIT.nPid1_4 = pPac_Pac_Sta->CRA_6.BIT.sBPID1_4;
;----------------------------------------------------------------------
        ldiu      437,ir1
        ldiu      16,r0
        ldiu      *+fp(ir1),ar1
	...
	.line	334
;----------------------------------------------------------------------
; 1144 | pCommStatus_Lic->BYTE_3.BIT.nPid2_1 = pPac_Pac_Sta->CRA_7.BIT.sBPID2_1;
; 1146 | //pCommStatus_Lic->BYTE_4.BIT.nDph = pPac_Pac_Sta->CRA_9.BIT.sBDPH;    
; 1147 | //pCommStatus_Lic->BYTE_4.BIT.nDpo = pPac_Pac_Sta->CRA_9.BIT.sBDPO;    
; 1148 | //pCommStatus_Lic->BYTE_4.BIT.nPei1 = pPac_Pac_Sta->CRA_7.BIT.sBPEI1;  
; 1149 | //pCommStatus_Lic->BYTE_4.BIT.nPei2 = pPac_Pac_Sta->CRA_7.BIT.sBPEI2;  
;----------------------------------------------------------------------
        ldiu      437,ir0
        ldiu      *+fp(ir0),ar0
        ldiu      439,ir0
...
L252:        
	.line	350
;----------------------------------------------------------------------
; 1160 | if(WORD_L(pCcp_Pac->phHdBuf.btSoh) == SOH &&  // CCP -> PAC ¿¡¼­ »óÅÂ Á
;     | ¤º¸¸¦ ÃßÃâÇÑ´Ù.                                                        
; 1161 |         WORD_L(pCcp_Pac->btEot) == EOT &&                              
; 1162 |         sizeof(CCP_PAC) == nRxPos &&                                   
; 1163 |         ((TWOBYTE_ASC2HEX(pCcp_Pac->phHdBuf.chSrcDev) == CCP_DEV_NO)||(
;     | TWOBYTE_ASC2HEX(pCcp_Pac->phHdBuf.chSrcDev) == CCP_BAKDEV_NO)) &&      
; 1164 |         //(TWOBYTE_ASC2HEX(pCcp_Pac->phHdBuf.chDestDev) == PAC_DEV_NO)&
;     | &                                                                      
; 1166 |         ConvAsc2Hex(pCcp_Pac->nCRC[0]) == BYTE_H(WORD_H(crc16_ccitt(&pC
;     | cp_Pac->phHdBuf.chSrcDev[0],nRxPos-6)))&&                              
; 1167 |         ConvAsc2Hex(pCcp_Pac->nCRC[1]) == BYTE_L(WORD_H(crc16_ccitt(&pC
;     | cp_Pac->phHdBuf.chSrcDev[0],nRxPos-6)))&&                              
; 1168 |         ConvAsc2Hex(pCcp_Pac->nCRC[2]) == BYTE_H(WORD_L(crc16_ccitt(&pC
;     | cp_Pac->phHdBuf.chSrcDev[0],nRxPos-6)))&&                              
; 1169 |         ConvAsc2Hex(pCcp_Pac->nCRC[3]) == BYTE_L(WORD_L(crc16_ccitt(&pC
;     | cp_Pac->phHdBuf.chSrcDev[0],nRxPos-6)))                                
; 1170 |         )                                                              
;----------------------------------------------------------------------
        ldiu      436,ir0
...
;*      Branch Occurs to L266 
	.line	363
;----------------------------------------------------------------------
; 1173 | if(m_Variable.m_chCarKind == 'A')                                      
;----------------------------------------------------------------------
        ldiu      @_m_Variable+342,r0
        cmpi      65,r0
        bne       L264
;*      Branch Occurs to L264 
	.line	365
;----------------------------------------------------------------------
; 1175 | m_Variable.m_nCarPos_Re = MAKE_WORD(ConvAsc2Hex(pCcp_Pac->sCI_Index[0][
;     | 0]),                                                                   
; 1176 |                                     ConvAsc2Hex(pCcp_Pac->sCI_Index[0][
;     | 1]));                                                                  
;----------------------------------------------------------------------
        ldiu      436,ir0
        ldiu      *+fp(ir0),ar0
	...
	.line	368
;----------------------------------------------------------------------
; 1178 | if(m_Variable.m_nCarPos_Re)                                            
; 1180 |         //m_Variable.m_nCarPos = m_Variable.m_nCarPos_Re; //150916_JDS 
;----------------------------------------------------------------------
        beq       L266
;*      Branch Occurs to L266 
        bu        L266
...
L264:        
	.line	374
;----------------------------------------------------------------------
; 1184 | else if(m_Variable.m_chCarKind == 'B')                                 
;----------------------------------------------------------------------
        ldiu      @_m_Variable+342,r0
        cmpi      66,r0
        bne       L266
;*      Branch Occurs to L266 
	.line	376
;----------------------------------------------------------------------
; 1186 | m_Variable.m_nCarPos_Re = MAKE_WORD(ConvAsc2Hex(pCcp_Pac->sCI_Index[1][
;     | 0]),                                                                   
; 1187 | 
...
;----------------------------------------------------------------------
	...
	.line	379
;----------------------------------------------------------------------
; 1189 | if(m_Variable.m_nCarPos_Re)                                            
; 1191 |         //m_Variable.m_nCarPos = m_Variable.m_nCarPos_Re; //150916_JDS 
;----------------------------------------------------------------------
L266:        
	.line	415
;----------------------------------------------------------------------
; 1225 | nOldUart3RxOneByteGapDelayTime = m_Variable.m_nUart3RxOneByteGapDelayTi
;     | me;                                                                    
;----------------------------------------------------------------------
        ldiu      @_m_Variable+290,r0
...
_GetLocalTimeToUTC:
	...
	.line	1
;----------------------------------------------------------------------
; 1231 | BOOL GetLocalTimeToUTC( DATE_TIME_PTR pLocal, int nHour, DATE_TIME_PTR
;     | pUTC )                                                                 
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	...
	.line	2
;----------------------------------------------------------------------
; 1233 | // ½Ã°£À» °è»êÇÏ´Â ºÎºÐ.                                               
;----------------------------------------------------------------------
	.line	4
;----------------------------------------------------------------------
; 1234 | int nHourT = pLocal->hour - nHour;                                     
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar0
        ldiu      *-fp(3),r0
	...
	.line	5
;----------------------------------------------------------------------
; 1235 | int nDayT = pLocal->day;                                               
;----------------------------------------------------------------------
        ldiu      *+ar0(3),r0
        sti       r0,*+fp(2)
	.line	6
;----------------------------------------------------------------------
; 1236 | int nLastDays = GetDaysOfMonth( pLocal->month, pLocal->year );
;     |                                                                        
;     |                                                                        
;----------------------------------------------------------------------
        ldiu      ar0,ar1
...
;----------------------------------------------------------------------
	...
	.line	8
;----------------------------------------------------------------------
; 1238 | int nYearT = 2000 + pLocal->year;                                      
; 1240 | // ½Ã°£ Á¤º¸¸¦ ±¸ÇÏ´Â ºÎºÐ.                                            
;----------------------------------------------------------------------
        ldiu      2000,r0
...
; 1247 | nDayT += 1;                                                            
; 1250 | // ÀÏÀÚ¸¦ ±¸ÇÏ´Â ºÎºÐ.                                                 
; 1251 | // ½Ã°£À» °è»êÇÏ¿© ÀÏÀÚ¸¦ º¯°æÇØ¾ßÇÏ´Â °æ¿ì.                           
;----------------------------------------------------------------------
        ldiu      1,r0
        addi      *+fp(2),r0
        sti       r0,*+fp(2)
...
;*      Branch Occurs to L276 
	.line	24
;----------------------------------------------------------------------
; 1254 | nDayT = GetDaysOfMonth( nMonthT, nYearT );                             
;----------------------------------------------------------------------
        ldiu      *+fp(5),r0
        push      r0
        ldiu      *+fp(4),r0
...
;----------------------------------------------------------------------
; 1269 | nMonthT = 1;                                                           
; 1273 | // °è»êµÈ ½ÃºÐÃÊ Á¤º¸¸¦ UTC ½Ã°£À¸·Î ÀÌµ¿½ÃÅ°´Â ºÎºÐ.                  
;----------------------------------------------------------------------
        ldiu      1,r0
        sti       r0,*+fp(4)
L279:        
	.line	44
;----------------------------------------------------------------------
; 1274 | pUTC->second = pLocal->second;                                         
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar0
        ldiu      *-fp(4),ar1
        ldiu      *ar0,r0
	...
	.line	45
;----------------------------------------------------------------------
; 1275 | pUTC->minute = pLocal->minute;                                         
;----------------------------------------------------------------------
        ldiu      *-fp(2),ar1
        ldiu      *-fp(4),ar0
        ldiu      *+ar1(1),r0
...
;----------------------------------------------------------------------
	...
	.line	49
;----------------------------------------------------------------------
; 1279 | pUTC->year = nYearT - 2000;                                            
; 1281 | return      (( 10 <= pUTC->year ) &&
;     |                                                                        
;     |                                   // 10³â ÀÌ»ó                         
; 1282 |                         (( 1 <= pUTC->month ) && ( pUTC->month <= 12 ))
;     |  &&                                                                    
;     |                   // ¿ù                                                
; 1283 |                         (( 1 <= pUTC->day )   && ( pUTC->day <= GetDays
;     | OfMonth(nMonthT, nYearT) )) &&                           // ÀÏ         
; 1284 |                         (( 0 <= pUTC->hour )  && ( pUTC->hour < 24 ))
;     |  &&                                                                    
;     |                   // ½Ã                                                
; 1285 |                         (( 0 <= pUTC->minute) && ( pUTC->minute < 60 ))
;     |  &&                                                                    
;     |                   // ºÐ                                                
;----------------------------------------------------------------------
        ldiu      2000,r0
	...
	.line	56
;----------------------------------------------------------------------
; 1286 | (( 0 <= pUTC->second) && ( pUTC->second < 60 )));
;     |                                                                  // ÃÊ 
;----------------------------------------------------------------------
        ldiu      *-fp(4),ar0
        ldiu      10,r0
...
_GetDaysOfMonth:
	...
	.line	1
;----------------------------------------------------------------------
; 1306 | int GetDaysOfMonth( int nMonth, int nYear )
...
_user_CarNoForCarKindToLon:
	.line	1
;----------------------------------------------------------------------
; 1342 | void user_CarNoForCarKindToLon()                                       
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	.line	3
;----------------------------------------------------------------------
; 1344 | m_Variable.m_nCarKindToLonCnt = 0;                                     
;----------------------------------------------------------------------
        ldiu      0,r0
...
_user_1msLoop:
	.line	1
;----------------------------------------------------------------------
; 1351 | void user_1msLoop()                                                    
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
	.line	3
;----------------------------------------------------------------------
; 1353 | m_Variable.m_nTxDbg1msTimer++;                                         
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	4
;----------------------------------------------------------------------
; 1354 | m_Variable.m_nUserDebug1msTimer++;                                     
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	6
;----------------------------------------------------------------------
; 1356 | m_Variable.m_nTest1msTimer++;                                          
;----------------------------------------------------------------------
        ldiu      1,r0
	...
	.line	9
;----------------------------------------------------------------------
; 1359 | if(m_Variable.m_nUart2RxOneByteGapDelayTime) m_Variable.m_nUart2RxOneBy
;     | teGapDelayTime--;                                                      
;----------------------------------------------------------------------
        ldi       @_m_Variable+289,r0
        beq       L318
...
L318:        
	.line	10
;----------------------------------------------------------------------
; 1360 | if(m_Variable.m_nUart3RxOneByteGapDelayTime) m_Variable.m_nUart3RxOneBy
;     | teGapDelayTime--;                                                      
;----------------------------------------------------------------------
        ldi       @_m_Variable+290,r0
        beq       L320
...
L320:        
	.line	12
;----------------------------------------------------------------------
; 1362 | if(m_Variable.m_nGiaRxCheck1msTimer) m_Variable.m_nGiaRxCheck1msTimer--
;     | ;                                                                      
;----------------------------------------------------------------------
...
L322:        
	.line	14
;----------------------------------------------------------------------
; 1364 | if(m_Variable.m_nCncsRxCheck1msTimer) m_Variable.m_nCncsRxCheck1msTimer
;     | --;                                                                    
;----------------------------------------------------------------------
...
SL25:	.byte	"CRA-FTP",0
SL26:	.byte	"CRA-CDT",0
SL27:	.byte	"Master CarNum : [%X].",13,10,0
SL28:	.byte	"CarNum : [%X].",13,10,0
;******************************************************************************
;* CONSTANT TABLE                                                             *
;******************************************************************************
	.sect	".const"
