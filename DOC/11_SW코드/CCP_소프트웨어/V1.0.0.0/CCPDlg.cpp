...
BOOL CCCPDlg::OnInitDialog()
{
	...
	wcscpy(m_arrCaption[CAPTION_TRAIN],					_T("Enter Train number") );	
	wcscpy(m_arrCaption[CAPTION_CREW],					_T("Press CREW to enter Crew ID") );	
	wcscpy(m_arrCaption[CAPTION_ROUTE],					_T("ROUTE") );
	...
	wcscpy(m_arrCaption[CAPTION_HELP_TRAIN],			_T("HELP / TRAIN") );			
	wcscpy(m_arrCaption[CAPTION_HELP_CREW],				_T("HELP / CREW") );				
	wcscpy(m_arrCaption[CAPTION_HELP_ROUTE],			_T("HELP / ROUTE") );			
	...
	wcscpy(m_arrCaption[CAPTION_HELP_MESSAGE],			_T("HELP / Form \"D\" Message") );	
	...
	wcscpy(m_arrCaption[CAPTION_CABON],					_T("Cab is not atcive") );
...
void CCCPDlg::InitTimer(void)
{
	...
	// FOR TEST
	//SetTimer( TIMER_CROSS_TIME, 1000, NULL );
}
...
//		m_strSchedule[3].Format( _T("%s %06d"), m_Route.GetDestinationCode(), m_Route.GetTrainNumber() );
		...
		m_strSchedule[14].Format( _T("T : %06d"), m_Route.GetTrainNumber() );
...
void CCCPDlg::DetectHardware(void)
{
	...
	DeviceIoControl( m_hEXS, IOCTL_EXS_GET_ADDR1, NULL, NULL, &dwReturnedA, sizeof(DWORD), &dwOnOff, NULL);
	DeviceIoControl( m_hEXS, IOCTL_EXS_GET_ADDR2, NULL, NULL, &dwReturnedB, sizeof(DWORD), &dwOnOff, NULL);
	
	if(dwReturnedA && dwReturnedB) 
	{
		m_Status.SetCCPID( ID_CCP_A );
	}
	else if(dwReturnedA && !dwReturnedB)
	{
		m_Status.SetCCPID( ID_CCP_A );
	}
	else if(!dwReturnedA && dwReturnedB) 
	...
	if( m_hEXS && !m_Status.IsUpdating() )
	{
		if(DeviceIoControl( m_hEXS, IOCTL_EXS_GET_CABON, NULL, NULL, &dwReturnedA, sizeof(DWORD), &dwOnOff, NULL ) )
		{						
			if(dwReturnedA)
			{
				if( !m_Status.IsSetCabonKey( ) )
				{
					...
					m_Status.SetUsing( true );
					SendMessage( WM_WRITE_LOG, (WPARAM)_T("SetUsing - CAB ON : [TRUE]") );

					m_Route.SetRoute();

					m_Status.SetUsing( false );
					SendMessage( WM_WRITE_LOG, (WPARAM)_T("SetUsing - CAB ON : [FALSE]") );
					//

					m_Status.SetBrightness( 9, this);

					WriteLog( _T("CAB KEY - Status Changed [ ON ] ") );
				}
			...
			else
			{
				if( m_Status.IsSetCabonKey( ) )
				{
					...
					WriteLog( _T("CAB KEY - Status Changed [ OFF ] ") );
				}
		...
		if(DeviceIoControl( m_hEXS, IOCTL_EXS_GET_CONKEY, NULL, NULL, &dwReturnedA, sizeof(DWORD), &dwOnOff, NULL ) )
		{						
			if(dwReturnedA)
			{
				if( !m_Status.IsSetConductorKey( ) ) 
				{
					...
					WriteLog( _T("CON KEY - Status Changed [ ON ] ") );
				}
			...
			else
			{
				if( m_Status.IsSetConductorKey( ) ) 
				{
					...
					WriteLog( _T("CON KEY - Status Changed [ OFF ] ") );
				}
...
bool CCCPDlg::UpdateCaption(UINT nCaptionIndex)
{
	...
	else
	{
		...
		else if( m_nCaptionIndex == CAPTION_CREW )
		{
...
void CCCPDlg::OnClick_Menu(UINT	nID)
{
	...
	else if( nID == IDC_BTN_CONFIG )	m_mgrPage.ShowPage(PAGE_ID_CONFIG);
...
void CCCPDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	{	
		...
		////	CAR_DATA car[2];
		////	car[0].CARN = 802;
		////	memset(car[0].DATA, 0xFF, sizeof(BYTE) * 8);

		////	car[1].CARN = 701;
		////	memset(car[1].DATA, 0xFF, sizeof(BYTE) * 8);

		////	car[0].DATA[1] = 0x40;

		////	m_Packet.SetCarData(2, car);
		////}
		////else
		////if( (step % 0x03) == 0x01 )
		////{
		////	CAR_DATA car[2];
		////	car[0].CARN = 802;
		////	memset(car[0].DATA, 0x00, sizeof(BYTE) * 8);

		////	car[1].CARN = 701;
		////	memset(car[1].DATA, 0x00, sizeof(BYTE) * 8);
