...
BOOL CPageHelp::OnInitDialog()
{
	...
	m_btnMessage.SetBitmapFromFile(_T(IDB_HELP_FORM_UP), _T(IDB_HELP_FORM_UP));
...
void CPageHelp::SetShowMode(UINT nShowMode)
{
	...
	else
	{
		...
		{
			strFind = _T("#TRAIN");
			m_stcTitle.SetWindowText( _T("TRAIN") );
		...
		{
			strFind = _T("#MESSAGE");
			m_stcTitle.SetWindowText(_T("Form “D” Message"));
		...
		m_edtContent.SetWindowText( m_strHelpContent.Mid( nStartIdx, nFinishIdx - nStartIdx ) );
