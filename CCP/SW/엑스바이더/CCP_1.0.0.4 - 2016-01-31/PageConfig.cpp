...
BOOL CPageConfig::OnInitDialog()
{
	...
	m_btnMenu[5].SetBitmapFromFile(_T(IDB_CONFIG_SPARE6_UP), _T(IDB_CONFIG_SPARE6_DN));
...
void CPageConfig::OnClick_Menu(UINT	nID)
{
	if( nID == IDC_BUTTON_SPEAKER )
	{
		...
		SetChangeDirect( SUB_MODE_SPEAKER );

		m_pParent->m_mgrPage.ShowPage(PAGE_ID_PASSWORD_VOL);
		m_pParent->SendMessage(WM_CAPTION_SET, CAPTION_CONFIG_SPEAKER);
	}
	else if( nID == IDC_BUTTON_LCD )	
	{	
		SetChangeDirect( SUB_MODE_MENU );
	...
	else if( nID == IDC_BUTTON_SIMULATION )
	{
		...
		m_pParent->m_mgrPage.ShowPage(PAGE_ID_PASSWORD_CI);
		m_pParent->SendMessage(WM_CAPTION_SET, CAPTION_CONFIG_CI);
	}
	else if( nID == IDC_BUTTON_PASSWORD )    
	{
		SetChangeDirect( SUB_MODE_MENU );

		m_pParent->m_mgrPage.ShowPage(PAGE_ID_PASSWORD_CHECK);
		m_pParent->SendMessage(WM_CAPTION_SET, CAPTION_CONFIG_PASSWORD);
	}
}
...
void CPageConfig::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	...
	if( nState == 1 )
	{
		if( m_nChangeDirect == 0 )
		{
			SetSubMode( SUB_MODE_MENU ); 
		}
		else
		{
			SetSubMode( m_nChangeDirect );
		}
...
{
	if( m_pParent->m_Status.GetCabSpeaker() == 0 )
	{
		OnClick_CabSpeaker( IDC_BTN_SPK_CAB_SOFT );
	}
	else
	{
		OnClick_CabSpeaker( IDC_BTN_SPK_CAB_NORMAL );
	}
