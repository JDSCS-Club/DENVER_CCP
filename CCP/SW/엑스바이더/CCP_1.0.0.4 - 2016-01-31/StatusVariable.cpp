...
//	m_nPEICarNo			= 0;	
	...
	CString strRevision;
	strRevision.LoadString(IDS_REVISION);

	_WCtoMB( strRevision.GetBuffer(0), m_szRevision );
	...
	if(!strFilePath.IsEmpty())
	{
		...
		if( bResult )
		{
			sprintf( m_szVersion, "%d%d%d%d%c%c",  HIWORD(vfi.dwFileVersionMS), LOWORD(vfi.dwFileVersionMS), HIWORD(vfi.dwFileVersionLS), LOWORD(vfi.dwFileVersionLS),
													m_szRevision[0], m_szRevision[1] );
		}
	}
...
void CStatusVariable::SetBrightness(UINT nBrightness, CWnd *pWnd)
{
	...
	{
		for(UINT i = 0; i< (nBrightness-m_nBrightness); i++)
		{
			BrightUp();
	...
	{
		for(UINT i = 0; i< (m_nBrightness-nBrightness); i++)
		{
			BrightDown();
...
BYTE CStatusVariable::GetCabSpeaker()
{
	return m_byCabSpeaker;
}
...
BYTE CStatusVariable::GetSpeakerValueEsp()
{
	BYTE	byRET = 0;
...
BYTE CStatusVariable::GetSpeakerValueInt()
{
	BYTE	byRET = 0;

	if( m_byCabSpeaker == 0)		byRET |= 0x40;
	else if( m_byCabSpeaker == 1 )	byRET |= 0x20;
	else if( m_byCabSpeaker == 2 )  byRET |= 0x10;

	if( m_byIntSpeaker == 0 )		byRET |= 0x04;
	else if( m_byIntSpeaker == 1 )	byRET |= 0x02;
	else if( m_byIntSpeaker == 2 )	byRET |= 0x01;
